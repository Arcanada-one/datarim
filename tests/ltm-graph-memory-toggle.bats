...
append_trailing_policy_section() {
    cat >> "$DR_PLUGIN_WORKSPACE/datarim/enabled-plugins.md" <<'EOF'
...
@test "LTM record and a trailing policy section coexist across mutation orders" {
    write_core_manifest
    append_trailing_policy_section
    run_plugin enable ltm-graph-memory
    [ "$status" -eq 0 ] || return 1
    grep -q '^- some-marker$' "$DR_PLUGIN_WORKSPACE/datarim/enabled-plugins.md" || return 1
    run_state
    ...
    [ "$status" -eq 0 ] && [ "$output" = "disabled" ] || return 1
    run_plugin enable ltm-graph-memory
    [ "$status" -eq 0 ] && grep -q '^- some-marker$' "$DR_PLUGIN_WORKSPACE/datarim/enabled-plugins.md"
}
