...
@test "wiring: commands/dr-plan.md invokes the detector under a MANDATORY cue" {
    CMD="$BATS_TEST_DIRNAME/../commands/dr-plan.md"
    [ -f "$CMD" ]
    run grep -n 'check-seam-integration-boundary.sh' "$CMD"
    [ "$status" -eq 0 ]
    inv=$(grep -n 'check-seam-integration-boundary.sh' "$CMD" | head -1 | cut -d: -f1)
    start=$(( inv > 8 ? inv - 8 : 1 ))
    run sed -n "${start},${inv}p" "$CMD"
    [[ "$output" == *MANDATORY* ]]
}

@test "wiring: the skill the detector's output points at exists" {
    # The advisory verdict cites skills/seam-vs-integration-boundary/SKILL.md;
    # a dangling pointer would make the gate documented-but-unresolvable.
    [ -f "$BATS_TEST_DIRNAME/../skills/seam-vs-integration-boundary/SKILL.md" ]
    run grep -c 'seam-vs-integration-boundary/SKILL.md' "$BATS_TEST_DIRNAME/../dev-tools/check-seam-integration-boundary.sh"
    [ "$status" -eq 0 ]
}
