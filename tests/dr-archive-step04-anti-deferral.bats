...
setup() {
    ...
    printf 'spaces/client/runbook.md\n' > "$TOUCHED"
}
...
## Layer 3b
The stale "21 containers" figure in spaces/client/runbook.md is informational,
...
@test "0.4 PASS: clean QA report + all-met expectations → both surfaces exit 0" {
    cat > "$WORK/datarim/qa/qa-report-FAKE-9200.md" <<'EOF'
## Layer 3b
All wishes met. Counter corrected in spaces/client/runbook.md, committed to origin.
EOF
    run "$PROSE_SCRIPT" --file "$WORK/datarim/qa/qa-report-FAKE-9200.md" \
        --touched-files "$TOUCHED" --root "$WORK"
    [ "$status" -eq 0 ]
