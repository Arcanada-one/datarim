...
setup() {
  ...
  mkdir -p "$SPACES_ROOT/arcanada" "$SPACES_ROOT/client"
...
YAML
  ...
  cat > "$SPACES_ROOT/client/space.yml" <<'YAML'
space:
  name: client
  status: active
autonomy:
...
@test "Client merge_main remains operator-gated" {
  gate client merge_main
...
@test "feature branch push is explicitly auto in both existing spaces" {
  ...
  [ "$status" -eq 0 ]
  gate client feature_branch_push
  [ "$status" -eq 0 ]
}
...
@test "non-dropping main update follows merge_main policy" {
  ...
  [ "$status" -eq 0 ]
  gate client force_push '{"drops_commits":false,"target_branch":"main"}'
  [ "$status" -eq 10 ]
}

@test "non-dropping feature force push follows feature branch policy" {
  gate client force_push '{"drops_commits":false,"target_branch":"feat/safe-rewrite"}'
  [ "$status" -eq 0 ] \
...
@test "DB operation with verified backup follows space policy" {
  ...
  [ "$status" -eq 0 ]
  gate client irreversible_db_op '{"backup_verified":true}'
  [ "$status" -eq 10 ]
}
...
@test "patch package release requires both release carve-out and space publish policy" {
  ...
  [ "$status" -eq 0 ]
  gate client public_package_release \
    '{"bump_level":"patch","escalate":false,"zero_x_breaking":false}'
  [ "$status" -eq 10 ]
}
