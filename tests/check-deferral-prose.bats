...
@test "BLOCK: deferral phrase about a touched file with no artefact" {
    cat > "$REPORT" <<'EOF'
## Layer 3b
The stale counter in spaces/client/runbook.md is informational, not a blocker,
...
@test "PASS: touched-file deferral citing a real FU-ID present in backlog" {
    cat > "$REPORT" <<'EOF'
## Layer 3b
The container count in spaces/client/runbook.md needs re-verification after a
...
@test "BLOCK: touched-file deferral citing a NON-existent FU-ID" {
    cat > "$REPORT" <<'EOF'
## Layer 3b
The stale counter in spaces/client/runbook.md is cosmetic, deferred to FAKE-7777.
EOF
    run "$SCRIPT" --file "$REPORT" --touched-files "$TOUCHED" --backlog "$BACKLOG" --tasks "$TASKS"
    [ "$status" -eq 1 ]
    [[ "$output" == *"runbook.md"* ]]
...
```
spaces/client/runbook.md:5: "informational" on touched file 'runbook.md'
...
But really, the stale counter in spaces/client/runbook.md is informational and
...
@test "PASS: deferral phrase and touched file are in DIFFERENT sentences of one line" {
    cat > "$REPORT" <<'EOF'
## Layer 3b
The 4 failures in tests/e2e/dev-1590.spec.ts are pre-existing baseline noise. Separately, spaces/client/runbook.md was updated and verified live.
EOF
    run "$SCRIPT" --file "$REPORT" --touched-files "$TOUCHED"
    [ "$status" -eq 0 ]
}
...
@test "BLOCK: deferral phrase and touched file in the SAME sentence still blocks" {
    cat > "$REPORT" <<'EOF'
## Layer 3b
The stale counter in spaces/client/runbook.md is pre-existing and out of scope. Separately, an unrelated topic follows here.
EOF
    run "$SCRIPT" --file "$REPORT" --touched-files "$TOUCHED"
    [ "$status" -eq 1 ]
    [[ "$output" == *"runbook.md"* ]]
...
It concerns spaces/client/runbook.md and will not be finished here.
EOF
    run "$SCRIPT" --file "$REPORT" --touched-files "$TOUCHED"
    [ "$status" -eq 1 ]
}
...
@test "BLOCK: a line with no sentence punctuation fails OPEN to the paragraph verdict" {
    cat > "$REPORT" <<'EOF'
## Layer 3b
spaces/client/runbook.md counter is pre-existing and out of scope for now
EOF
    run "$SCRIPT" --file "$REPORT" --touched-files "$TOUCHED"
    [ "$status" -eq 1 ]
}
