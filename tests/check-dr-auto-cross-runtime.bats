...
@test "wiring: a CI job runs the gate itself, not only this suite" {
    # A gate landed without enforcement wiring is documented-but-unwired. Assert a
    # workflow invokes the script, so deleting the job fails here instead of
    # silently disarming the check.
    WF="$BATS_TEST_DIRNAME/../.github/workflows/dev-tools-lint.yml"
    [ -f "$WF" ]
    run grep -c 'check-dr-auto-cross-runtime.sh' "$WF"
    [ "$status" -eq 0 ]
    # and it must be a blocking job — not parked under continue-on-error
    run python3 - "$WF" <<'PY'
import sys, re
s = open(sys.argv[1], encoding='utf-8').read()
i = s.index('dr-auto-cross-runtime:')
# the job block runs to the next top-level job key (two-space indent) or EOF
m = re.search(r'\n  [a-z][a-z0-9-]*:\n', s[i:])
block = s[i:i+m.start()] if m else s[i:]
sys.exit(1 if 'continue-on-error' in block else 0)
PY
    [ "$status" -eq 0 ]
}
