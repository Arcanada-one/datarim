...
EOF
    ...
    printf '%s\n' \
"1700000000000-0	id=uuid-1	ts=2026-07-21T00:00:00Z	type=audit	task_id=DEMO-0001	reason=ran ssh dev@host-devs cat /home/dev/secret/key -- token=SECRET123abc against 10.1.2.3	outcome=success" \
...
@test "redaction: no sensitive trace survives into the emitted dataset" {
    ...
    # mesh hostname
    ! printf '%s' "$output" | grep -q 'host-devs'
    # IPv4 address
...
@test "redact_trace scrubs secrets, paths, hosts and IPs" {
    source "$REDACT_LIB"
    local out
    out=$(redact_trace 'ssh dev@host-prod token=abc123XYZ path /var/lib/foo host db.arcanada.club ip 192.168.0.5')
    ! printf '%s' "$out" | grep -q 'abc123XYZ'
    ! printf '%s' "$out" | grep -q '/var/lib/foo'
    ! printf '%s' "$out" | grep -q 'host-prod'
