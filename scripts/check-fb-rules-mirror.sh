...
# own ecosystem `AGENTS.md` at rank-1 mandate level. The framework ships the
...
#
# This checker verifies a consumer `AGENTS.md` mirrors that canonical text by
...
#      Rules" appears (case-sensitive) in the consumer AGENTS.md.
#   2. Rule coverage   — every `rule_id` in the canonical policy YAML is cited
#      verbatim (FB-1 .. FB-8, exact case) in the consumer AGENTS.md.
#
# It is stack-agnostic and ecosystem-agnostic: no consumer repo path, IP, or
# hostname is hard-coded. The consumer AGENTS.md is supplied by the caller
# (positional arg or env), the canonical YAML is resolved inside this repo.
#
# Usage:
#   ./scripts/check-fb-rules-mirror.sh <consumer-AGENTS.md>
#   ./scripts/check-fb-rules-mirror.sh --quiet <consumer-AGENTS.md>
...
#   FB_RULES_CONSUMER_CLAUDE    consumer AGENTS.md (alternative to positional arg)
#
# Exit codes:
...
# Consumer AGENTS.md: positional arg first, then env fallback.
if [ -z "$CONSUMER" ]; then
    CONSUMER="${FB_RULES_CONSUMER_CLAUDE:-}"
fi
if [ -z "$CONSUMER" ]; then
    echo "ERROR: consumer AGENTS.md path required (positional arg or FB_RULES_CONSUMER_CLAUDE)." >&2
    echo "Usage: $0 [--quiet] <consumer-AGENTS.md>" >&2
    exit 2
fi
if [ ! -f "$CONSUMER" ]; then
    echo "ERROR: consumer AGENTS.md not found: $CONSUMER" >&2
    exit 2
fi
...
if [ "$drift" -ne 0 ]; then
    echo "DRIFT: consumer AGENTS.md does not fully mirror canonical FB-rules." >&2
    ...
    echo "       text into the consumer ecosystem AGENTS.md at rank-1 mandate level" >&2
    echo "       before enabling the dr-orchestrate plugin." >&2
    exit 1
fi

$QUIET || echo "IN SYNC: consumer AGENTS.md mirrors canonical FB-rules."
exit 0
