...
blueprint_revision: 3170ea0aea46ca2497ab2c3fc16cd6408725d1d1
...
3. Resolve private authoritative source/child manifests and public monotonic head anchors through governance-signed provider registrations that bind bootstrap/provider payload/signature tuples plus immutable argv/result schema locators and digests. Enforce algorithm/key-or-issuer/subject/audience/namespace/role/scope/validity/revocation/verifier-digest and no-downgrade rules. Every accepted event must resolve and verify its detached `event-attestation` against the exact event preimage/hash and current registry revision using the event's exact mapped authority key—not the anchor-authority key. Implement the closed provider-native control journal and query/reserve/append/result wires: zero-hash genesis, exact control records reachable from the protected ref, null-sequence receipt-template digest/idempotency binding, one durable outstanding reservation, gap-free FIFO sequences, append-time authority revalidation, and state-aware descendant/cursor replay. Implement the sole compromise escape as offline-quorum `reservation-abort`: it consumes the ordinal with a governance-signed non-evidence abort receipt, preserves the terminal plan, then allows revocation/replan; cancellation, ordinal reuse, or signer substitution remain impossible. Mutate forged-role attestations, changed-request/idempotency/template/signature bytes, parallel/out-of-order reservation, cancellation/skip, invalid abort quorum, pre-revocation reservation/post-revocation append, missing control blobs, and nonzero genesis RED. Invoke adapters with argv and `shell=False`, a fixed cwd, allowlisted environment, executable digest, integer-minute deadline, output cap, authentication metadata, and strict result schema; deadline expiry sends SIGTERM to the process group and returns the normalized exit-143 record.
...
10. Resolve/refetch an exact anchor-signed provider-state response with no outstanding reservation and embed it in closed `datarim/evidence/TUNE-0585/migration-input.json`; create signed `source-snapshot.json`, then create immutable `migration-plan.json` with the exact PRD `migrate --plan --mode genesis` CLI using committed input/snapshot/registry, exact authorization-event commit, null epoch/compatibility/removal inputs, bootstrap-pinned time, registered signer-provider ID, and receipt output. The plan pins the input provider head/last-consumed sequence and closed distinct event-signer map; every item names its mapped signer, full event, attestation ID, and unsigned receipt template. Before any reservation, apply runs/persists the exact all-signer challenge preflight and verifies every proof. It then classifies current provider state: first apply requires the pinned head; resume permits only a validated descendant/cursor reconstructed from deterministic plan/recovery keys, already-created byte-identical records, and complete foreign interleaves. Divergence, foreign outstanding reservation, registry/signer-map change, or terminal abort fails. Each new reservation binds the null-sequence template digest; apply fills only provider sequence, obtains/verifies the mapped authority signature through the closed signer-provider wire, and compare-appends/refetches the receipt. The unique initial manifest is transition zero/planned with the exact plan event set in `unpublished`; after each consumed/refetched receipt its state changes directly to `refetched` and a separate recovery receipt is published; installation changes all and only states to `bound`. Exact phase/set/field equivalences and durable pre-CAS `registry-prepared` transition are enforced. Failure after publication leaves immutable refetched-but-unbound `pending-orphan` receipts; an identical retry binds them after install. Re-run against the same provider history to prove byte-identical plan/ledger and all existing remote bytes; mutate legal interleaving to prove cursor continuation without changing ledger semantics. Independently verify every field/digest against freeze/authorization and derive `genesis_unanchored`. Validate with `dev-tools/check-customer-delivery.sh --task TUNE-0585 --stage plan --root . --format json`. This conversion must complete before Task 4 records canonical evidence.
...
1. RED-test the exact `migrate --plan --mode legacy-v0` and `migrate --apply` wire formats for genuine init-task/append-log import, closed source-snapshot/migration-input/provider-state/plan/recovery/CAS schemas, signed exhaustion and pagination, retroactive Overview recovery, legacy evidence text, no ambient clock/unregistered network access, source/input/registry/signer-map divergence, invalid initial provider state, invalid descendant/cursor or valid foreign interleave, deterministic ledger replay, null-sequence template binding, signer preflight omission/failure, gap-free FIFO reservation/retry, offline-quorum abort-and-replan, forbidden cancellation/skip/substitution, remote-published/local-missing recovery, the unique exact-event-set `unpublished` transition-zero manifest, omitted/extra/reordered states, bound-before-install or installed-with-unbound state, every invalid phase-field equivalence or skipped transition/signature, existing identical/non-identical targets, receipt partial failure, sibling-temp/fsync/rename failure, anchor-before-registry-CAS ordering, timeout-after-anchor recovery, durable `registry-prepared` successor/signature/request tuple, post-CAS/pre-local-record crash lookup, exact CAS owner/full-request digest/result/expected sequence+digest and immutable successor registry/signature tuple, monotonic adoption, and dual-read sunset.
...
| `datarim/evidence/TUNE-0585/migration-input.json` | closed schema, source/freeze/authorization digest cross-check, anchor-signed no-outstanding provider-state replay, and deterministic planning |
...
- checked: 84 unique artifact/edit/validation path references, including 77 local paths and 7 external-provider paths;
- present: 35 local paths (34 exact-base targets plus this plan);
- expected new: 42 local paths, each explicitly marked `[to-be-created]`;
