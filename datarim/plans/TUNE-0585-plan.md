...
blueprint_revision: d30b7fecf77679fc922ef3f415faaef0c5b035b8
...
1. RED-test the exact `migrate --plan --mode legacy-v0` and `migrate --apply` wire formats for genuine init-task/append-log import, closed source-snapshot/migration-input/plan/recovery/CAS schemas, signed exhaustion and pagination, retroactive Overview recovery, legacy evidence text, no ambient clock/unregistered network access, source/input/registry drift, deterministic byte replay, gap-free FIFO receipt reservation/retry and cancellation, remote-published/local-missing recovery, the unique unreserved transition-zero manifest, every invalid state/phase cross-constraint or skipped transition/signature, existing identical/non-identical targets, receipt partial failure, sibling-temp/fsync/rename failure, anchor-before-registry-CAS ordering, timeout-after-anchor recovery, exact CAS owner/full-request digest/result/expected sequence+digest and immutable successor registry/signature tuple, monotonic adoption, and dual-read sunset.
...
8. After tag/receipt/current-rules verification, construct Datarim consumer-registry sequence `n+1` with predecessor digest/sequence from the genesis registry and non-null `canon_epoch` binding release version, full tag object, peeled commit, ledger blob/head, and anchor receipt. Governance signs it, and the registry owner publishes it through the exact `registry-cas` request binding both successor-registry and governance-signature locators/digests plus the full request digest. A clean resolver refetches/verifies the authenticated CAS result and both immutable successor blobs. Only this external successor makes the release canonical and permits the universal canon complete verdict; it is never appended to its subject ledger and no unplanned next Git release/commit is required. A downstream consumer registry records the receipt locator/digest, tag object, peeled commit, ledger blob/head/count, and release version when it adopts the canon. Talomnia must first register and probe its own dedicated `git-protected-ref-v1` append branch before strict mode.
