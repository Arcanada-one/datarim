...
blueprint_revision: 8931f7961b03738f335e98563849e2457a5cdca8
...
7. Create/sign/publish/refetch the authorization receipt over the marker. First invoke the registry-fixed source provider's read-only observation profile to produce/sign exact source-revision bytes; do not let it write derived state. Materialize and governance-sign the closed `genesis-input` request binding the contract/freeze/authorization/registry/recorded-at/receipt-output tuple plus the source provider's signed immutable sequence/head/manifest locator/digest/signature/item-count revision; commit the source revision/signature plus request/signature together as signed `attestation-input-request`; their relative locators resolve only through that containing commit supplied by the later invocation. Invoke the reviewed bootstrap's `genesis-input apply` with the signed request commit/locator/digest and exact bytes to produce the exhausted source snapshot and migration input, then `genesis-plan` to produce the plan containing `implementation_authorized` with `authorization_request_commit` equal to the already signed request commit. Persist its request-bound governance-signed result/signature, exercise lookup/retry against the same historical revision, and commit those two plus the snapshot, input, and plan outputs as signed `attestation-plan` and resolve the plan through its exact Git-object commit/locator. Run each event's independently registered `caller-sign-v1` provider through `genesis-prepare` to create every deterministic authorization request/caller signature and their closed manifest, then commit those bytes alone as signed `attestation-requests`. Only after that commit exists, dispatch `genesis-attest` to one isolated fixed-policy authority process per plan event using the exact `--event-id`/provider binding against the immutable plan/request commits; a signer mapped to several events receives several isolated invocations. Persist every receipt/signature/authenticated decision partial, and use the bootstrap's authorized `genesis-bundle` operation to create both the exact partials manifest and bundle without appending the planned event receipts. The plan separately freezes the sole recovery caller and anchor signer that apply may invoke for post-transition dynamic recovery; no generic process holds or selects caller or authority keys. Commit immutable bindings, partials, manifest, bundle, authorization event, and cleanup as the separate signed authorization-event commit. The first general source/test implementation commit must descend from it; later code only applies frozen pre-work bytes and performs the first planned event append after durable transition-zero.
8. Verify exact ordering `spec-pair → bootstrap-authorization → bootstrap-red → bootstrap-source → bootstrap-review → trust-request → trust-provision → spec-review-request → contract-base → freeze → authorization-marker → authorization-request → attestation-input-request → attestation-plan → attestation-requests → authorization-event`, all signatures/receipts/append-result sequences and Git ancestry from `d27b15f`, a clean branch, current permanent-branch protection, and absence of every disposable provider resource before Task 2.

**Validation:** bootstrap RED/GREEN and exact-head review pass; both governance-signed genesis requests and operation results validate; provider feasibility semantics pass; hosted one-commit append/rejection/read-only-lookup/resolve and cleanup are independently signed and replayable; every caller proof, immutable request commit, receipt, and authority-decision result validates against the pre-existing registry; normal and anchored/null-anchor recovery registry-CAS paths pass; `git verify-commit` passes for all sixteen ordered commits; `git merge-base --is-ancestor d27b15f HEAD` passes; permanent authority protection is current; init-task and expectations gates exit 0; and both specification reports plus the bootstrap quality report end in their required passing verdicts.
...
| Genesis | sixteen ordered commits before general source, including authorized/reviewed bootstrap TCB plus immutable trust/spec/authorization/attestation-input requests; development-only raw RED, present-day signed mutation, protected-ref epoch | V-AC-9 |
...
| `tests/customer-delivery-bootstrap.bats` | intended missing-executable RED; caller-sign and genesis plan/prepare/attest/bundle wires; common one-CAS provider/control/lookup recovery; differential vectors |
...
| `config/customer-delivery-bootstrap-provider-v1.json` | authorization/review/signature closure plus pinned caller-sign, genesis attest/bundle, mutation-control/lookup, executable/request/result/argv/env/cwd replay |
...
2. Commit bootstrap authorization, bootstrap RED, bootstrap source, bootstrap review, genesis trust request, trust provision, spec-review request, contract base, genesis freeze, authorization marker, authorization request, genesis input request, attestation plan, attestation requests, and authorization event in the exact declared order; run the concrete provider feasibility probe at its prescribed point.
...
- deterministic inventory at this revision: 107 referenced artifact targets = 35 present local paths in the reviewed planning worktree + 65 absent local paths explicitly owned as `[to-be-created]` + 7 non-path `[external-provider-role]` targets; local path inventory is therefore 100. Of the 35 present paths, 31 existed at declared exact base `d27b15f390265fef1fc95c31a28677a9664acb98` and 4 are specification-chain artifacts added afterward. The pre-`/dr-do` validator must recompute and match these counts after any path edit.
