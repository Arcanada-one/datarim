...
blueprint_revision: e0ecf0c893452029e31d842accc2e76e52ea35cb
...
| `datarim/delivery/TUNE-0585.genesis-trust-request.json` | exact reviewed-pair/auth/review, catalog/registry/identity/provider/private-handle/rule specs and no host-assigned fixed point |
...
| `datarim/delivery/TUNE-0585.genesis-input-request.json` | exact contract/freeze/authorization/source-provider/registry/time/output input before snapshot/planning |
...
- all seven provider-owned paths are explicitly marked `[external-provider-artifact]` and excluded from local existence checks;
...
- deterministic inventory at this revision: 100 unique paths total = 35 present local paths in the reviewed planning worktree + 58 absent local paths explicitly owned as `[to-be-created]` + 7 provider-owned `[external-provider-artifact]` paths; of the 35 present paths, 31 existed at declared exact base `d27b15f390265fef1fc95c31a28677a9664acb98` and 4 are specification-chain artifacts added afterward. The pre-`/dr-do` validator must recompute and match these counts after any path edit.
