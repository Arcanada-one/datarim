...
blueprint_revision: 0081fe2a65c4338c70c8809f555690a68d1f4444
...
3. Resolve private authoritative source/child manifests and public monotonic head anchors through governance-signed provider registrations that bind the immutable bootstrap payload/signature tuple. Enforce algorithm/key-or-issuer/subject/audience/namespace/role/scope/validity/revocation/verifier-digest and no-downgrade rules. Every accepted event must resolve and verify its detached `event-attestation` against the exact event preimage/hash and current registry revision using the event's exact mapped authority key—not the anchor-authority key. Implement the closed provider-native control journal: zero-hash genesis, canonical reservation/consumption records reachable from the protected ref, full-request digest/idempotency equality, receipt ID/type/registry binding, one durable outstanding reservation, gap-free FIFO sequences, and append-time authority revalidation. There is no cancellation/skip path: recovery must consume the byte-identical reservation, and permanent loss of its exact signer is a declared hard gate. Mutate forged-role attestations, changed-request idempotency reuse, parallel/out-of-order reservation, attempted cancellation/skip, pre-revocation reservation/post-revocation append, missing control blobs, and nonzero genesis RED. Invoke adapters with argv and `shell=False`, a fixed cwd, allowlisted environment, executable digest, integer-minute deadline, output cap, authentication metadata, and strict result schema; deadline expiry sends SIGTERM to the process group and returns the normalized exit-143 record.
...
10. Create closed `datarim/evidence/TUNE-0585/migration-input.json` and signed `source-snapshot.json`, then create immutable `migration-plan.json` with the exact `migrate --plan --mode genesis` CLI, passing locally resolved committed bootstrap/input/snapshot/registry, exact authorization-event commit, null epoch/compatibility/removal inputs, bootstrap-pinned time, registered signer-provider ID, receipt output, and the authenticated provider's exact current control head/last-consumed sequence. The plan carries the closed distinct event-signer map, and every item names its exact mapped signer ID, full canonical event, attestation ID, and complete unsigned receipt template with deterministic issued time/idempotency key; concatenation exactly reconstructs planned ledger bytes. Verify every input/output digest, then run the exact `migrate --apply` CLI with plan digest, input, source snapshot, registry, `--signer-provider <registered-provider-id>`, receipt output, and ledger output. Apply rejects provider-head/sequence or signer-map drift, then uses the non-common FIFO protocol to reserve one stable sequence/public compare token at a time, fills only provider sequence, obtains/verifies the exact mapped authority signature, and compare-appends/refetches that event receipt. The unique initial manifest is transition zero/planned with the exact plan event set in `unpublished`; transient reservations live only in authenticated provider state. After each receipt is consumed/refetched, its state changes directly to `refetched` and a separate recovery receipt is published; target installation changes all and only states to `bound`. Exact phase/set/field equivalences and the durable pre-CAS `registry-prepared` transition are enforced. Failure after receipt publication leaves immutable refetched-but-unbound `pending-orphan` receipts; they cannot pass a stage, recovery queries reservations/receipts by deterministic keys, and an identical retry binds them after install. Re-run plan/apply to prove byte-identical/idempotent plan, ledger, native reservation/control results/signatures, receipts, and recovery transitions; independently verify every field/digest against freeze/authorization and derive `genesis_unanchored`. Validate with `dev-tools/check-customer-delivery.sh --task TUNE-0585 --stage plan --root . --format json`. This conversion must complete before Task 4 records canonical evidence.
...
**Post-merge release validation:** initialize an empty resolver; fetch only `refs/tags/v$(tr -d '\n' < VERSION)`; check out the fetched tag detached; run `git verify-tag` with the release authority allowed for namespace `git`; separately extract and verify the closed annotation receipt under `datarim-release-v1`; resolve the full tag object, peeled commit, tree, and `datarim/delivery/TUNE-0585.jsonl` blob OIDs; extract and rehash the ledger; refetch the external quality receipt and current release-ruleset response; validate signer/schema/registry/full OIDs/byte length/event count/head/evidence digest/current rules; invoke the validator from the checked-out tagged tree as `"$resolver/dev-tools/check-customer-delivery.sh" --task TUNE-0585 --stage release --root "$resolver" --quality-receipt <resolved-receipt> --anchor-receipt <resolved-tag-annotation> --format json`; then publish/refetch the signed registry successor and its exact governance-signature blob with exact predecessor and `canon_epoch` through `registry-cas`, verifying the full request digest and returned tuple from a clean resolver. Never invoke the feature-worktree validator for tagged bytes. All commands and successor verification must exit 0 before V-AC-9 or canonical completion passes.
