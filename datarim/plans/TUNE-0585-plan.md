...
blueprint_revision: 56063c37e51a9ffaaf64d0385bcc6bf042ac4945
...
Genesis first uses a directly governance-signed, path-confined bootstrap authorization because the framework cannot self-authorize an executable that does not yet exist. Bootstrap RED, source, and exact-head review are separate signed commits; only then may the reviewed bootstrap provision trust. The remaining sequence separates each immutable request from a later commit-bound invocation authorization, then trust provision, contract base, freeze, authorization marker, attestation plan, attestation requests, and authorization event because no artifact may name the commit that contains itself. The permanent receipt authority and signed registry exist before the canonical exact-blob specification receipt; the contract-base commit makes the requirement-scoped manifest addressable; the freeze binds that receipt, manifest, and provider proof; the marker names the freeze; the attestation-plan commit freezes semantic context; the attestation-requests commit freezes every caller-signed request; and the authorization-event commit freezes every receipt/policy decision before general source. General implementation begins only in a descendant of the authorization-event commit. Raw TDD and the bootstrap ceremony are preserved but are not canonical runner evidence. After the runner exists, present-day mutation/restoration supplies signed canonical RED/GREEN, and the verified protected release plus registry-successor CAS becomes the monotonic `canon_epoch`.
...
- Create: `datarim/delivery/TUNE-0585.bootstrap-authorization.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.bootstrap-authorization.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-trust-request.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-trust-request.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-trust-invocation.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-trust-invocation.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-trust-result.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-trust-result.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-input-request.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-input-request.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.source-revision.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.source-revision.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.source-manifest.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.source-manifest.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-input-invocation.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-input-invocation.json.sig` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-input-result.json` `[to-be-created]`
- Create: `datarim/delivery/TUNE-0585.genesis-input-result.json.sig` `[to-be-created]`
- Create: `tests/customer-delivery-bootstrap.bats` `[to-be-created]`
- Create: `scripts/customer-delivery-bootstrap.py` `[to-be-created]`
- Create: `config/customer-delivery-bootstrap-wire-v1.schema.json` `[to-be-created]`
- Create: `datarim/evidence/TUNE-0585/bootstrap-red.txt` `[to-be-created]`
- Create: `datarim/evidence/TUNE-0585/bootstrap-review.json` `[to-be-created]`
- Create: `datarim/evidence/TUNE-0585/bootstrap-review.json.sig` `[to-be-created]`
...
- Create: `datarim/evidence/TUNE-0585/spec-review-request/` `[to-be-created]`
...
- Create: `datarim/evidence/TUNE-0585/attestation-requests/` `[to-be-created]`
...
- Create: `datarim/evidence/TUNE-0585/authorization-request/` `[to-be-created]`
...
1. Resolve `approved_pair_commit` as the exact signed HEAD on which both preliminary subordinate reviewers returned YES; it must contain the exact plan blob under review and separately record the PRD blueprint revision from frontmatter. Using only the pre-existing governance key and `ssh-keygen`, create/sign the closed bootstrap-authorization object that pins the exact allowed bootstrap executable/wire-schema/test paths plus non-empty role/skill/blueprint/constraint/policy/success-criterion arrays. Commit it alone as `bootstrap-authorization`. Add the bootstrap Bats contract and retained intended missing-executable output, run it RED at that assertion, and commit only test/evidence as `bootstrap-red`. Then implement only `scripts/customer-delivery-bootstrap.py` plus the exact `config/customer-delivery-bootstrap-wire-v1.schema.json`, make the bootstrap suite GREEN against every schema fragment, and commit those two reviewed TCB files as `bootstrap-source`. Dispatch a fresh subordinate exact-head compliance/quality review; preserve its report plus the closed bootstrap-review record and direct governance signature under `datarim-bootstrap-review-v1` in a separate `bootstrap-review` commit. The record binds exact authorization/report locators and digests, bootstrap commit/tree/executable/wire-schema/test/RED digests, finding digest, and verdict; later registration binds its locator/digest and signature locator/digest. Any finding returns to a new RED/source/review chain; no amend/rebase. Until the signed review passes, the bootstrap may not create keys, trust records, provider state, or general source.

   After the passing bootstrap review, create/sign `config/customer-delivery-bootstrap-provider-v1.json` binding the exact authorization/review/signature locators/digests, source commit/executable/wire-schema digests, fixed pre-existing governance result-signer reference, Python/system executable versions/digests, fixed cwd/env, and exact Git-object-plus-JSON-Pointer request/result schema locators/digests for `canonicalize`, `caller-sign`, `activate-key`, `genesis-trust`, `genesis-input`, `genesis-plan`, `genesis-prepare`, `genesis-attest`, `genesis-bundle`, `authorize-and-sign`, `publish-blob`, `resolve-blob`, `lookup-provider-operation`, `append-receipt`, the byte-identical `lookup-receipt` argv alias, `resolve-receipt`, and `registry-cas`. Materialize and governance-sign the closed genesis-trust request with every catalog/registry/policy/caller/signer/recovery/provider spec, create-once private-store handle, immutable validity/scope/policy field, deterministic host rule target, fixed result output paths, and zero-bypass rule; commit only bootstrap registration/signature plus trust request/signature as signed `trust-request` descending from bootstrap review. Then create/sign the closed invocation-authorization binding that exact request commit/locator/digest/signature tuple and allowed apply/lookup actions; commit it/signature alone as signed `trust-invocation`. Invoke registered `genesis-trust apply` only through that later authorization commit/locator/digest/signature; copying the request to another commit fails. Before any mutation it must fsync the closed private transition journal at the registration-fixed state root plus operation/request-digest child key; each key/file/ref/ruleset step is create-or-verify, atomically journaled, and resumable. Exercise `lookup`, crash after every step, identical retry, changed-request collision, and foreign-resource rejection. It provisions distinct non-repository `caller-sign-v1` key processes and authority signers for `registry-owner`, `spec-reviewer`, `atomicity-reviewer`, `evolution-owner`, `evolution-reviewer`, `quality-reviewer`, `evidence-runner`, `project-verifier`, `source-provider`, `deployment-provider`, `visual-provider`, `reconciliation-authority`, `anchor-authority`, and `release-authority`, plus the disjoint offline recovery quorum; creates/read-backs the permanent protected ref/ruleset; and emits only public provider/catalog/registry artifacts plus a request-bound governance-signed result; a complete result/signature is persisted and journaled before response. Store each provider's non-self-referential registration payload and governance signature under `config/customer-delivery-provider-registrations/`; every payload retains exact bootstrap registration/signature locators/digests. Commit only the closed operation result/signature and its output set—provider registrations/signatures, public reciprocal caller/signer assignments, catalog, Datarim genesis registry, derived allowed-signers view, provider-registration manifest, and detached signatures—as signed `trust-provision`. The registry binds governance fingerprint `SHA256:a3hhUtadjU+KNZ1zI2LQuS88vb+t7YnQUiA4IDNbLgM`, unique `(authority_provider_id, caller_subject) -> (caller_provider_id, signer_id)` mappings, each protected-ref provider's one stable `result_signer_id` whose key may rotate without changing provider or signer identity, fixed roles/policies/namespaces/scopes, and receipt authority `refs/heads/datarim/customer-delivery-receipts-v1` with exact host rules and no bypass. In disposable fixtures, prove trust-request tampering/key leakage fails; namespace schemas and caller envelopes are disjoint; common publish/append/CAS controls and response-loss lookup agree; normal CAS binds provider/owner; ordinary old+new continuity and quorum recovery each prepare a pending replacement key and possession proof while the predecessor remains active, then finalize the stable slot only after accepted-CAS refetch; and prior-catalog quorum recovery replaces compromised governance/registry-owner/anchor keys without consulting them in anchored and exact null-anchor states.
2. Obtain a subordinate exact-blob review of the unchanged PRD/plan pair after the trust-provision commit. Materialize the caller-signed authorization request and semantic spec-review receipt template, bind exact pair/blobs, registry, trust commit, findings, verdict, fixed caller, and spec-reviewer provider/policy/key/namespace/scope, and commit the request/proof alone as signed `spec-review-request`. Only then may the spec-reviewer provider resolve that Git-object locator, return the receipt signature/authenticated decision, and atomically append/refetch the complete request/receipt/signature/decision tuple in one protected-ref commit. Separately verify its append-result provider sequence. Each later event attestation follows the same fixed-authority policy; every protected-ref blob/append/CAS/lookup/resolve result uses only that provider record's one fixed result signer. If either reviewer returns NO, revise/commit specification, replace dependent trust revision without rewriting history, and repeat both reviews. No general implementation source before two YES verdicts bind the same pair; the only earlier source is the narrowly authorized/reviewed bootstrap TCB.
3. Before any general Task 2 implementation source or RED test is created, complete one hard outer eight-minute feasibility probe of the concrete genesis and ongoing profiles; the narrowly authorized bootstrap RED/source/review exception in Step 1 necessarily precedes this gate. A dedicated one-minute negative-control command spawns a child and grandchild; it passes only when the supervisor signals the process group and records normalized exit `143`, raw wait status, `termination_signal:15`, `timed_out:true`, `forced_kill`, and no surviving descendant. A second SIGTERM-resistant control must exercise the five-second grace/forced-kill branch without leaking a descendant. Expected timeout-control exits do not fail the outer probe; any unexpected exit `143` on an ordinary command does. Record a monotonic absolute outer deadline; before each launch require `deadline_minutes * 60 + 5` seconds of termination grace plus the fixed 10-second evidence-fsync reserve to fit, otherwise fail without launching. Fsync each command record before continuing so the eight-minute watchdog cannot be the normal cleanup path. The replay below covers signer behavior and immutable-tag resolution; the same report must also contain a disposable hosted append/resolve probe that proves current admin/write/read credential scope. At probe startup, before allocating a new probe ID or making any hosted write, scan canonical-byte order under `<private_state_root>/probes/`; every prior manifest not `cleaned` must be reconciled and cleaned first. Resolve branch create/push/delete response loss with exact `git ls-remote` ref/OID readback and ruleset create/delete response loss with exact nonce/target API listing; delete the ruleset first, prove its absence, then delete the branch and prove its absence, persisting each observation. Any ambiguous duplicate, changed target/OID, unavailable cleanup credential, or surviving prior resource blocks a new probe. Before the first hosted mutation, create and fsync `<private_state_root>/probes/<probe_id>.json` and its parent directory. This private durable recovery manifest has exactly `schema_version:uint=1`, `probe_id:id128`, `request_sha256:sha256`, `repository_id:id128`, `remote_url:https_url`, `branch_ref:text`, `ruleset_target:text`, `ruleset_nonce:id128`, `ruleset_id:uint|null`, `expected_branch_oid:git_oid|null`, `state` (`prepared`, `branch-created`, `ruleset-created`, `published`, `cleanup-pending`, or `cleaned`), `observations[]`, and `updated_at:utc_seconds`. Each observation has exactly `action` (`branch-read`, `branch-write`, `branch-delete`, `ruleset-read`, `ruleset-create`, or `ruleset-delete`), `expected:text|null`, `observed:text|null`, `raw_artifact_sha256:sha256`, `recorded_at:utc_seconds`, and `verdict` (`match`, `absent`, or `conflict`); it contains no credential or secret. The branch/ref, ruleset target, and unique nonce are persisted in `prepared` state before either create. After each API/Git response, atomically replace/fsync the manifest; every create/push/delete response is reconciled through the matching read operation before retry; require exactly one exact ruleset/no-bypass match and one exact branch OID or absence as appropriate, persist the observation, or fail closed. Then create a uniquely named temporary branch from the reviewed pair head, create/read back a GitHub ruleset targeting only that exact branch with `deletion` and `non_fast_forward` rules and no bypass actor, push a first commit containing a synthetic immutable ledger blob, and push one append-only fast-forward. Then issue an actual remote forced update with `git push --force-with-lease=<full-ref>:<known-remote-oid> origin <ancestor>:<full-ref>` and an actual remote deletion with `git push origin :<full-ref>`; both must reach GitHub and fail with the named ruleset/rule ID, not client fast-forward checks, auth, hooks, or malformed argv. Clone into a new empty repository and recompute the full branch commit OID, ledger Git blob OID, byte length, event count, and event head from fetched bytes. Retain redacted raw API/Git response bytes under `provider-raw/` and record their digests plus every exact argv, exit code, start/end time, and attribution assertion. The disposable branch remains protected until cleanup; the permanent receipt branch remains protected indefinitely. Any failed API/write/read/rejection/resolve blocks the freeze and all source work.
...
6. After verifying the signed freeze and every committed digest, delete only the disposable ruleset, then its now-unprotected branch, re-read both to prove absence, and append the immutable cleanup event. The permanent receipt authority remains protected. Commit `datarim/delivery/TUNE-0585.authorization-marker.json` separately; it binds the freeze commit, exact clean repository/base, and dirty-inventory digest but does not self-name. Then create the caller-signed authorization request and semantic authorization receipt over that marker and commit their exact bytes alone as signed `authorization-request`; only after this commit may the governance provider sign or append the authorization receipt.
7. Create/sign/publish/refetch the authorization receipt over the marker. First invoke the registry-fixed source provider's read-only observation profile to produce/sign exact exhausted source-manifest bytes and the sequence/head revision bytes that locates them; do not let it write derived state. Materialize and governance-sign the closed `genesis-input` request binding the contract/freeze/authorization/registry/recorded-at/receipt-output tuple plus the source provider's signed immutable sequence/head/manifest locator/digest/signature/item-count revision; commit source manifest/signature, source revision/signature, and request/signature together as signed `attestation-input-request`; every relative locator resolves only through that exact commit. Then create/sign the closed invocation-authorization over that request commit/locator/digest/signature tuple and commit it/signature alone as signed `attestation-input-authorization`. Invoke the reviewed bootstrap's `genesis-input apply` only through the latter authorization commit/locator/digest/signature to produce the exhausted source snapshot and migration input, then `genesis-plan` to produce the plan containing `implementation_authorized` with `authorization_request_commit` equal to the already signed request commit. Persist its request-bound governance-signed result/signature, exercise authenticated absent/in-progress/complete lookup plus crash/retry after journal creation, source resolution, each output, result persistence, and response loss against the same historical revision, and commit those two plus the snapshot, input, and plan outputs as signed `attestation-plan` and resolve the plan through its exact Git-object commit/locator. Run each event's independently registered `caller-sign-v1` provider through `genesis-prepare` to create every deterministic authorization request/caller signature and their closed manifest, then commit those bytes alone as signed `attestation-requests`. Only after that commit exists, dispatch `genesis-attest` to one isolated fixed-policy authority process per plan event using the exact `--event-id`/provider binding against the immutable plan/request commits; a signer mapped to several events receives several isolated invocations. Persist every receipt/signature/authenticated decision partial, and use the bootstrap's authorized `genesis-bundle` operation to create both the exact partials manifest and bundle without appending the planned event receipts. The plan separately freezes the sole recovery caller and anchor signer that apply may invoke for post-transition dynamic recovery; no generic process holds or selects caller or authority keys. Commit immutable bindings, partials, manifest, bundle, authorization event, and cleanup as the separate signed authorization-event commit. The first general source/test implementation commit must descend from it; later code only applies frozen pre-work bytes and performs the first planned event append after durable transition-zero.
8. Verify exact ordering `spec-pair → bootstrap-authorization → bootstrap-red → bootstrap-source → bootstrap-review → trust-request → trust-invocation → trust-provision → spec-review-request → contract-base → freeze → authorization-marker → authorization-request → attestation-input-request → attestation-input-authorization → attestation-plan → attestation-requests → authorization-event`, all signatures/receipts/append-result sequences and Git ancestry from `d27b15f`, a clean branch, current permanent-branch protection, and absence of every disposable provider resource before Task 2.

**Validation:** bootstrap RED/GREEN and exact-head review pass; both governance-signed genesis requests, commit-bound invocation authorizations, authenticated lookup unions, and operation results validate; provider feasibility semantics pass; hosted one-commit append/rejection/read-only-lookup/resolve and restart cleanup are independently signed and replayable; every caller proof, immutable request commit, receipt, authority-decision result, continuity/recovery key activation, and replacement-key possession proof validates against the pre-existing registry/catalog; normal and anchored/null-anchor recovery registry-CAS paths pass; `git verify-commit` passes for all eighteen ordered commits; `git merge-base --is-ancestor d27b15f HEAD` passes; permanent authority protection is current; init-task and expectations gates exit 0; and both specification reports plus the bootstrap quality report end in their required passing verdicts.
...
4. Add evidence/disposition mutants for fabricated/manual/hash-only receipts; wrong signer role/scope/namespace/audience; authority/blob request schema reuse; missing/wrong caller provider; ambiguous or non-reciprocal caller/result signer; missing/rewritten/invalid provider decision signature or policy/context result; expired/revoked/downgraded registry; caller proof encoded as authority signer; stale/wrong `effective_registry_sequence`; old-registry append after revocation CAS; normal-rotation continuity failure; insufficient/duplicate/wrong-catalog recovery quorum; recovery that consults a compromised governance/registry-owner/anchor key; second-based/zero/out-of-range deadlines; process-group/grandchild timeout not reported as SIGTERM exit `143`; missing/forged raw wait status; SIGTERM-resistant forced-kill descendant leak; forged customer identity; vacuous RED; assertion mismatch; generic/404 live response; matrix-cell alias; missing each matrix cell; wrong deployed SHA; and accepted/returned/rejected/deferred/withdrawn semantics.
5. Add durability mutants for concurrent/divergent writers, duplicate retry, stale lock, short write/disk full, process death around rename/blob publication, operation/control/object-set/head/path mismatch across blob/receipt/CAS, blob-CAS response loss with caller unavailable, forged generic lookup or introducing commit, fsync failure, anchor timeout-after-accept, unanchored recovery, anchor recursion, every forbidden supersession target/actor, duplicate target, shared replacement, branch, merge, cycle, older/inactive/already-superseded replacement, missing invalidation descendants, countermand selection by non-monotonic index, same-predecessor conflicts, unpaired reconciliation records, wrong conflict cardinality/shared predecessor/chosen membership, and unauthorized reconciliation.
6. In the migration suite, RED-test the exact plan/prepare/per-event-attest/bundle/apply wires, all closed source/input/plan/request/partial/bundle/recovery/blob-publication/CAS schemas, per-event caller/provider selection and `caller-sign` namespace/schema enforcement, repeated authority across several events, exact partial-manifest construction, `authorization_request_commit`, dynamic recovery caller/context publication, common provider-control sequencing, signed result lookup after response loss, and normal/recovery registry-CAS authorization envelopes. Include deterministic replay, first-run remote-state reconstruction, authorized transition zero, phase/set/bound equivalence, crash/timeout/idempotency paths, key-only/governance compromise recovery and concurrent revocation-CAS ordering, and strict-adoption mutants before any general implementation exists.
7. Run both focused suites before creating the implementation and retain the expected missing-core and missing-migration assertions as raw development RED in `datarim/evidence/TUNE-0585/red/` `[to-be-created]`. They must fail at their intended assertions, not setup, but are explicitly not canonical runner attestations.

**Validation:** `bats tests/customer-delivery-gate.bats tests/customer-delivery-migration.bats` exits non-zero at both intended missing executable assertions, not setup; evidence metadata says `development_only: true`.
...
- Read-only input: `datarim/evidence/TUNE-0585/migration-input.json` (owned by Task 1)
- Read-only input: `datarim/evidence/TUNE-0585/source-snapshot.json` (owned by Task 1)
- Read-only input: `datarim/evidence/TUNE-0585/migration-plan.json` (owned by Task 1)
...
- Modify: `tests/customer-delivery-migration.bats`
...
3. Resolve private authoritative source/child manifests and public monotonic heads through governance-signed provider registrations that bind bootstrap authorization/review signatures plus provider payload/signature tuples and immutable argv/request/result schema locators/digests. Register caller-key processes independently; enforce unique reciprocal `(authority_provider_id, caller_subject) -> (caller_provider_id, signer_id)` mappings, namespace-specific authority/blob request schemas, and one fixed result signer per protected-ref provider. Neither request nor CLI may select a caller, signer, key, role, policy, namespace, or provider outside the frozen record. Verify caller, receipt, result, and authenticated decision signatures. Implement the common blob/receipt/CAS control/index/path/head/sequence contract, caller-authenticated multi-blob publication/resolution, generic read-only operation lookup, and two-phase journaled `activate-key` prepare/rebind/finalize with full-request idempotency (including output paths), an exact identity-state document atomically compare-swapping the whole active/pending tuple, closed state/nullability invariants, crash-after-promotion replay, and a per-identity request/journal-bound supersession proof accepted only after signed complete-ancestry search, available for every caller/signer/governance activation independently of lookup-signer substitution and keyed by a closed slot-lineage digest across original/rebind aliases, finds no accepted CAS for that activation and reports a third authoritative registry locator/sequence/digest, permanently fencing every request containing the old-registry activation; any historical acceptance resumes finalize even after later registry transitions. Keep provider control head selection inside the registry-CAS mutation lock so pointer publication cannot stale or self-reference the request; enforce exact initialized/latest expected and replacement registry locator/sequence/digest equality across request, activation prepares, provider state, result, signature tuple, and idempotency. Retain old+new continuity or prior-catalog quorum authority, pending replacement-key possession proof, predecessor-active CAS, recovery result signing by the unchanged active signer when it is not rotated, offline-quorum rebind of an already installed safe pending handle/generation to new recovery successor bytes when its active predecessor becomes compromised, with a create-exclusive request reservation before signed bytes, fresh recovery result/proof, closed rebind-prepared/rebound crash states, and a safe normal-vs-recovery CAS race, narrowly transition-authorized absent/present lookup by the proven pending signer for exact normal or recovery signer rotation, and post-CAS slot promotion. Implement no-reservation receipt append/lookup/resolve and pointer-envelope registry-CAS with normal owner authorization, exact ordinary/governance continuity bundles retained only in CAS tuples with null successor revocation locators/digests as normal revocation authority, and prior-catalog key/governance recovery deltas. Enforce full-wire idempotency, append-time revocation, response-loss reconstruction without caller reuse, and no sequence on pre-CAS failure. Mutate schema/namespace substitution, missing/ambiguous caller/result signers, changed proofs/locators/bytes, object-set/control/head/path mismatch, concurrent append, crashes before/after every CAS, revoked publication, nonzero genesis, missing/mismatched activation proof, stable-identity/provider drift, compromised-key consultation, concurrent non-identical prepares, missing/changed supersession proof, same-registry absent supersession, foreign provider-control publication before CAS, different-registry supersession with multiple delayed requests sharing the old activation, a differently keyed accepted request whose current registry equals the prepared replacement, an accepted sibling hidden behind a later registry descendant, serialized supersession of separate identities from one multi-activation CAS, and same-registry recovery rebind after active result-signer compromise, incompatible ordinary/recovery successor bytes, competing normal/recovery CAS winners queried through both losing aliases, cross-variant slot-lineage mismatch, fixed vectors for exact public-key JSON string comments/newlines/normalization, same public key through a different handle, crash before/after rebind reservation/result/proof journaling, and rebind journal retry; every accepted case must finalize rather than supersede; normal and recovery pending-signer absent/present lookup, missing genesis/governance identity-state initialization, undefined or partially updated identity state, crash after whole-document slot promotion before journal completion, changed output paths under one activation idempotency key, recovery-CAS response loss before signer finalization, recovery without result-signer rotation, stale-CAS active-slot change, failed-CAS old-key reactivation, stale revocation registry boundary, untyped normal revocation authority, and forbidden recovery deltas. Differentially verify every closed bootstrap request/result vector. Invoke adapters argv-only with fixed cwd/env/digest, integer-minute deadline/output cap, and strict schema; timeout returns normalized exit 143.
...
6. Expose `init`, `append`, `caller-sign`, `activate-key`, `publish-blob`, `resolve-blob`, `lookup-provider-operation`, `lookup-receipt`, `resolve-receipt`, `registry-cas`, `validate --stage`, `derive`, and all five `migrate --plan|--prepare|--attest|--bundle|--apply` operations with the exact PRD arguments: `authorization_request_commit`, registry-resolved per-event caller processes, `--event-id`, frozen authority provider, repeated partials plus partials output, immutable plan/request commits and locators, receipt output, epoch, compatibility, and removal inputs. Private caller/authority keys remain available only to their separately configured provider processes. The shell wrapper maps contract findings to 1, usage/integrity to 2, and required-provider failure to 3.
...
10. Apply the exact pre-implementation migration input/source snapshot/committed plan/attestation partials/bundle already frozen in the authorization-event commit; do not regenerate or re-sign them. Resolve the exact plan/request commits and locators, replay every event caller/context/policy proof, prove read-only lookup reports every planned event absent, and durably create transition-zero with all states `authorized` before the first planned append. Then append/refetch each event receipt through the one-CAS protocol. For every recovery transition, use only the plan-fixed recovery caller to publish/refetch the preimage/request/caller-proof blob set, then the fixed anchor signer to authorize and append/refetch the recovery receipt. If local state is lost after any CAS, reconstruct the unique prefix through `lookup-provider-operation` and deterministic blob/request/receipt/result keys without inventing or post-hoc attributing history. Installation changes all and only states to `bound`. Enforce phase/set/field equivalences and durable pre-CAS `registry-prepared`. Failure leaves refetched-but-unbound pending-orphan receipts; identical retry binds them. Re-run to prove byte-identical plan/partials/bundle/ledger/semantic/recovery bytes and stable signed provider results and introducing commits under foreign interleaving. Independently verify every field/digest against freeze/authorization and derive `genesis_unanchored`. Validate with `dev-tools/check-customer-delivery.sh --task TUNE-0585 --stage plan --root . --format json`. This conversion must complete before Task 4 canonical evidence.

**Validation:** `bats tests/customer-delivery-gate.bats tests/customer-delivery-migration.bats`; `python3 -m py_compile scripts/customer-delivery-ledger.py`; `dev-tools/check-customer-delivery.sh --task TUNE-0585 --stage plan --root . --format json`.
...
### Task 5: Harden legacy compatibility and sunset
...
1. Before changing any legacy/dual-read path, extend the already-GREEN migration suite with independent RED cases for genuine init-task/append-log byte import, agent-authored retroactive source, imported implementation/test/deploy prose, exact one-release compatibility, removal-version expiry, pre-epoch classification, monotonic adoption, and strict-mode reversion. These are compatibility deltas only; the five migration wires and recovery/provider protocols were already RED in Task 2 and implemented in Task 3.
...
4. Require an exact release/tag-object/commit/ledger/receipt `canon_epoch`, name one compatibility release and removal version in the immutable migration plan, and fail closed after removal or on any strict-mode reversion. Reuse Task 3's already-tested immutable event publication, recovery, anchor-before-CAS, signed normal registry-owner envelope, authenticated CAS result, and crash lookup; compatibility code may not add a parallel provider path.
5. Re-run focused migration and core suites GREEN and retain the compatibility-only RED evidence before its source change.
...
- Resolve externally: `artifact-role:registry-successor` `[external-provider-role]`
- Resolve externally: `artifact-role:registry-successor-signature` `[external-provider-role]`
- Resolve externally: `artifact-role:registry-cas-request` `[external-provider-role]`
- Resolve externally: `artifact-role:registry-cas-request-signature` `[external-provider-role]`
- Resolve externally: `artifact-role:registry-cas-result` `[external-provider-role]`
- Resolve externally: `artifact-role:registry-cas-result-signature` `[external-provider-role]`
- Resolve externally: `artifact-role:registry-cas-recovery` `[external-provider-role]`
...
| Genesis | eighteen ordered commits before general source, including authorized/reviewed bootstrap TCB plus immutable trust/spec/authorization/attestation-input requests and separate commit-bound invocation authorizations; development-only raw RED, present-day signed mutation, protected-ref epoch | V-AC-9 |
...
| `datarim/delivery/TUNE-0585.bootstrap-authorization.json` | exact allowed bootstrap paths, six non-empty capability classes, clean base/pair binding, expiry |
| `datarim/delivery/TUNE-0585.bootstrap-authorization.json.sig` | direct governance fingerprint and bootstrap-authorization namespace verification |
| `datarim/delivery/TUNE-0585.genesis-trust-request.json` | exact reviewed-pair/auth/review, catalog/registry/policy/caller/signer/recovery/provider/private-handle/journal/rule specs and no host-assigned fixed point |
| `datarim/delivery/TUNE-0585.genesis-trust-request.json.sig` | governance signature under `datarim-genesis-trust-request-v1` before trust mutation |
| `datarim/delivery/TUNE-0585.genesis-trust-invocation.json` | exact request commit/locator/digest/signature binding and apply/lookup action allowlist before mutation |
| `datarim/delivery/TUNE-0585.genesis-trust-invocation.json.sig` | direct governance signature under `datarim-genesis-trust-invocation-v1` in the later invocation commit |
| `datarim/delivery/TUNE-0585.genesis-trust-result.json` | committed request locator/digest, durable transition digest, deterministic public outputs, hosted readback, and idempotent completion |
| `datarim/delivery/TUNE-0585.genesis-trust-result.json.sig` | direct fixed-governance result signature under `datarim-genesis-trust-result-v1` |
| `datarim/delivery/TUNE-0585.genesis-input-request.json` | exact contract/freeze/authorization/registry/time/output plus co-committed signed immutable source revision before snapshot/planning |
| `datarim/delivery/TUNE-0585.genesis-input-request.json.sig` | governance signature under `datarim-genesis-input-request-v1` before input/source construction |
| `datarim/delivery/TUNE-0585.source-revision.json` | fixed source provider's read-only sequence/head/exhausted-manifest/item-count observation, co-committed before input construction |
| `datarim/delivery/TUNE-0585.source-revision.json.sig` | source-provider signature and namespace verification over exact revision bytes |
| `datarim/delivery/TUNE-0585.source-manifest.json` | exact exhausted historical source bytes resolved from the authenticated request commit |
| `datarim/delivery/TUNE-0585.source-manifest.json.sig` | source-provider manifest signature under `datarim-source-manifest-v1` |
| `datarim/delivery/TUNE-0585.genesis-input-invocation.json` | exact input request commit/locator/digest/signature binding and apply/lookup allowlist |
| `datarim/delivery/TUNE-0585.genesis-input-invocation.json.sig` | direct governance signature under `datarim-genesis-input-invocation-v1` in the later authorization commit |
| `datarim/delivery/TUNE-0585.genesis-input-result.json` | committed request binding, immutable signed source revision, deterministic input/snapshot outputs, and retry equality |
| `datarim/delivery/TUNE-0585.genesis-input-result.json.sig` | direct fixed-governance result signature under `datarim-genesis-input-result-v1` |
| `tests/customer-delivery-bootstrap.bats` | intended missing-executable RED; commit-bound genesis apply/lookup unions and crash journals; caller-sign/key-activation/continuity/recovery possession; plan/prepare/attest/bundle wires; common one-CAS provider/control/lookup recovery; differential vectors |
| `scripts/customer-delivery-bootstrap.py` | stdlib compile, every closed bootstrap request/result schema GREEN, source-path confinement, exact-head subordinate review |
| `config/customer-delivery-bootstrap-wire-v1.schema.json` | exact reviewed bootstrap request/result/namespace/policy/provider-manifest subschema digest and differential vectors |
| `datarim/evidence/TUNE-0585/bootstrap-red.txt` | retained intended assertion/exit before bootstrap source commit |
| `datarim/evidence/TUNE-0585/bootstrap-review.json` | closed schema, exact authorization/report locators/digests, bootstrap commit/tree/source/test/RED/finding/verdict |
| `datarim/evidence/TUNE-0585/bootstrap-review.json.sig` | direct governance signature under `datarim-bootstrap-review-v1` over exact review bytes |
...
| `datarim/evidence/TUNE-0585/spec-review-request/` | immutable caller request/signature/receipt template, Git-object locator, and trust-commit binding |
...
| `datarim/evidence/TUNE-0585/authorization-request/` | immutable caller request/signature/receipt template and marker/freeze binding before provider use |
...
| `datarim/evidence/TUNE-0585/attestation-requests/` | exact plan/request commit locators, caller request/signature set/order, and request-manifest digest |
...
| `artifact-role:registry-successor` | provider result resolves its content-addressed `blobs/<sha256>` locator; verify predecessor/epoch schema and CAS-result digest equality |
| `artifact-role:registry-successor-signature` | provider result resolves its content-addressed blob locator; verify governance namespace/signature over exact successor digest |
| `artifact-role:registry-cas-request` | provider result/control resolves the exact immutable request blob and verifies prepared idempotency/successor/anchor tuple equality |
| `artifact-role:registry-cas-request-signature` | provider result/control resolves the immutable signature blob and verifies registry-owner namespace/role/scope |
| `artifact-role:registry-cas-result` | normative `results/registry-cas/<idempotency_key>.json` locator resolves the anchor-authenticated result and successor tuple |
| `artifact-role:registry-cas-result-signature` | normative `results/registry-cas/<idempotency_key>.json.sig` locator verifies the fixed anchor-authority result signer |
| `artifact-role:registry-cas-recovery` | provider publication result resolves prepared/committed recovery blobs and crash-lookup replay; no semantic pseudo-path is used |
...
| `config/customer-delivery-datarim-registry.json` | closed consumer schema, catalog link, reciprocal caller/signer/result-signer bindings, successor-registry key revocation, adoption checks |
...
| `config/customer-delivery-bootstrap-provider-v1.json` | authorization/review/signature closure plus pinned caller-sign/key-activation, commit-bound genesis apply/lookup, attest/bundle, mutation-control/lookup, private state root, executable/request/result/argv/env/cwd replay |
...
2. Commit bootstrap authorization, bootstrap RED, bootstrap source, bootstrap review, genesis trust request, trust invocation authorization, trust provision, spec-review request, contract base, genesis freeze, authorization marker, authorization request, genesis input request, genesis input invocation authorization, attestation plan, attestation requests, and authorization event in the exact declared order; run the concrete provider feasibility probe at its prescribed point.
...
All existing edit/validation targets above must resolve in the exact-base worktree. Every absent local path is explicitly marked `[to-be-created]`; each non-path external artifact role is explicitly marked `[external-provider-role]` and must resolve at runtime to the registered provider's normative `blobs/`, `controls/`, `indexes/`, or `results/` locator returned by an authenticated result. No semantic external label is a provider path. No deprecated target is allowed. Run the plan-path validator again after any path revision and before `/dr-do`.

PATH VALIDATION

- every current local target was resolved against the reviewed planning worktree, not mislabelled as present at the older exact base;
- every absent local target is explicitly marked `[to-be-created]` in its owning task;
- all seven external provider artifact roles are explicitly marked `[external-provider-role]`, excluded from local path checks, and required to resolve to normative authenticated provider locators;
- unexpected missing targets: none;
- deprecated edit targets: none (marker hits describe retired concepts inside live current files, not retired target paths);
...
- deterministic inventory at this revision: 113 referenced artifact targets = 35 present local paths in the reviewed planning worktree + 71 absent local paths explicitly owned as `[to-be-created]` + 7 non-path `[external-provider-role]` targets; local path inventory is therefore 106. Of the 35 present paths, 31 existed at declared exact base `d27b15f390265fef1fc95c31a28677a9664acb98` and 4 are specification-chain artifacts added afterward. The pre-`/dr-do` validator must recompute and match these counts after any path edit.
