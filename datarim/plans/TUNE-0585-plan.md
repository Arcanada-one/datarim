...
blueprint_revision: f04781f4dc8ae6d0953c70695332d2812912f691
...
7. Create/sign/publish/refetch the authorization receipt over the marker. With the registered reviewed bootstrap executable, create exact migration input/source snapshot/plan containing the `implementation_authorized` event and `authorization_request_commit` equal to the already signed request commit, then commit those three artifacts as signed `attestation-plan`. Resolve the plan through its exact Git-object commit/locator. Run each event's registry-fixed caller-proof provider to create every deterministic authorization request/caller signature and their closed manifest, then commit those bytes alone as signed `attestation-requests`. Only after that commit exists, dispatch one isolated fixed-policy authority process per plan event using the exact `--event-id`/provider binding against the immutable plan/request commits; a signer mapped to several events receives several isolated invocations. Persist every receipt/signature/authenticated decision partial, and use `bundle` to create both the exact partials manifest and bundle without appending the planned event receipts. The plan separately freezes the sole recovery caller and anchor signer that apply may invoke for post-transition dynamic recovery; no generic process holds or selects caller or authority keys. Commit immutable bindings, partials, manifest, bundle, authorization event, and cleanup as the separate signed authorization-event commit. The first general source/test implementation commit must descend from it; later code only applies frozen pre-work bytes and performs the first planned event append after durable transition-zero.
...
6. In the migration suite, RED-test the exact plan/prepare/per-event-attest/bundle/apply wires, all closed source/input/plan/request/partial/bundle/recovery/blob-publication/CAS schemas, per-event caller/provider selection, repeated authority across several events, exact partial-manifest construction, `authorization_request_commit`, dynamic recovery caller/context publication, and normal/recovery registry-CAS authorization envelopes. Include deterministic replay, first-run remote-state reconstruction, authorized transition zero, phase/set/bound equivalence, crash/timeout/idempotency paths, compromise recovery, and strict-adoption mutants before any general implementation exists.
...
3. Resolve private authoritative source/child manifests and public monotonic heads through governance-signed provider registrations that bind bootstrap authorization/review signatures plus provider payload/signature tuples and immutable argv/request/result schema locators/digests. Enforce the unique reciprocal caller-to-signer mapping; neither request nor CLI may select a caller, signer, key, role, policy, namespace, or provider outside the frozen record. Verify caller, receipt, and authenticated decision signatures. Implement caller-authenticated multi-blob publication/read-only resolution, no-reservation one-CAS receipt append/lookup/resolve, and pointer-envelope registry-CAS with normal registry-owner authorization plus exact prior-catalog recovery delta. Share a gap-free provider sequence across mutating operations; enforce stable full-wire idempotency, append-time revocation, crash reconstruction, and no sequence on pre-CAS failure. Mutate spoofed/ambiguous caller mappings, changed proofs/locators/bytes, arbitrary payload/policy/context, blob-set mismatch, concurrent append, crashes before/after every CAS, revoked publication, missing control blobs, nonzero genesis, compromised-key consultation, and forbidden recovery deltas. Differentially verify every closed bootstrap request/result vector. Invoke adapters argv-only with fixed cwd/env/digest, integer-minute deadline/output cap, and strict schema; timeout returns normalized exit 143.
...
6. Expose `init`, `append`, `publish-blob`, `resolve-blob`, `lookup-receipt`, `resolve-receipt`, `registry-cas`, `validate --stage`, `derive`, and all five `migrate --plan|--prepare|--attest|--bundle|--apply` operations with the exact PRD arguments: `authorization_request_commit`, registry-resolved per-event callers, `--event-id`, frozen authority provider, repeated partials plus partials output, immutable plan/request commits and locators, receipt output, epoch, compatibility, and removal inputs. The shell wrapper maps contract findings to 1, usage/integrity to 2, and required-provider failure to 3.
...
| `tests/customer-delivery-bootstrap.bats` | intended missing-executable RED, closed bootstrap wires, one-commit provider, caller proof, recovery, differential vectors |
| `scripts/customer-delivery-bootstrap.py` | stdlib compile, bootstrap suite GREEN, source-path confinement, exact-head subordinate review |
...
| `config/customer-delivery-bootstrap-provider-v1.json` | authorization/review/signature locator/digest closure plus pinned executable/request/result schema/argv/env/cwd replay |
