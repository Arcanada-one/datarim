...
blueprint_revision: ff2d108339b73529da126d459df0aeab8cab0b0b
...
7. Create/sign/publish/refetch the authorization receipt over the marker. With the registered reviewed bootstrap executable, create exact migration input/source snapshot/plan containing the `implementation_authorized` event and commit them as a signed `attestation-plan` commit. Resolve the plan through its exact Git-object commit/locator. Run the fixed caller-proof providers to create every deterministic authorization request/caller signature and their closed manifest, then commit those bytes alone as signed `attestation-requests`. Only after that commit exists, dispatch one isolated fixed-policy authority process per mapped signer against the immutable plan/request commits; persist every receipt/signature/authenticated decision partial, aggregate the exact bundle, but do not append the planned event receipts yet. No generic process holds or selects caller or authority keys. Commit both immutable commit/locator bindings, partials, bundle, authorization event, and cleanup as the separate signed authorization-event commit. The first general source/test implementation commit must descend from it; later code only applies frozen pre-work bytes and performs the first planned event append after durable transition-zero.
...
3. Resolve private authoritative source/child manifests and public monotonic head anchors through governance-signed provider registrations that bind bootstrap/provider payload/signature tuples plus immutable argv/result schema locators/digests. Bind every caller to its own key/provider scopes and each signer to one fixed provider, policy digest, caller subject, role/key/namespaces/scopes; neither request can select those fields. Every accepted event verifies the immutable caller-signed request, mapped authority signature, and provider's separate `datarim-authority-decision-v1` signature over the fixed context/policy result. Implement the closed no-reservation one-commit append/result wire and read-only lookup wire: complete semantic receipt/signature/decision first, atomic provider sequence assignment, protected-ref predecessor/result linkage, append-time revocation validation, stable idempotency lookup, and no sequence on pre-CAS failure. Mutate spoofed/unauthorized caller, changed caller proof, arbitrary payload, rewritten unsigned policy metadata, wrong context/policy/role, changed locator or other idempotency bytes, concurrent append, crash before/after CAS, revoked-before-publication signature, missing control blobs, and nonzero genesis RED. Implement normal continuity and prior-catalog offline-quorum recovery CAS, including compromised governance/registry-owner/anchor replacement without consulting those keys and the tightly constrained null-anchor genesis case. Differentially verify every bootstrap vector against the general implementation. Invoke adapters argv-only with fixed cwd/env/digest, integer-minute deadline/output cap, and strict schema; timeout returns normalized exit 143.
...
6. Expose `init`, `append`, `lookup-receipt`, `validate --stage`, `derive`, and all five `migrate --plan|--prepare|--attest|--bundle|--apply` operations with the exact PRD arguments, including plan/request commits and locators, registry, caller/authority provider, receipt-output, epoch, compatibility, and removal inputs. The shell wrapper maps contract findings to 1, usage/integrity to 2, and required-provider failure to 3.
...
1. RED-test the exact plan/prepare/attest/bundle/apply wires for genuine init-task/append-log import, closed source/input/plan/request/partial/bundle/recovery/CAS schemas, signed exhaustion, retroactive recovery, legacy text, no ambient clock/unregistered network access, plan/request commit-locator mismatch, source/input/registry/caller/signer/provider/policy drift, unauthorized caller/arbitrary payload, missing or rewritten authenticated provider-decision result, exact request/attestation-set mismatch, deterministic ledger replay, atomic append idempotency and foreign interleave, revoked-before-publication signature, remote-published/local-missing recovery, absent-local-manifest with remote-state first-run ambiguity, exact `authorized` transition zero, omitted/extra/reordered states, bound/install equivalence, invalid phase/signature, identical/different targets, partial receipt/write/fsync failure, anchor-before-CAS, timeout-after-anchor, durable `registry-prepared`, post-CAS crash lookup, immutable successor registry/signature tuple, normal rotation continuity, prior-catalog offline-quorum compromise recovery, monotonic adoption, and dual-read sunset.
...
| `datarim/evidence/TUNE-0585/bootstrap-review.json` | exact bootstrap commit/tree/finding/verdict and reviewed-pair binding |
| `datarim/evidence/TUNE-0585/bootstrap-review.json.sig` | direct governance signature over passing subordinate review record |
...
- numeric totals are generated from the committed plan at the pre-`/dr-do` gate rather than copied as drift-prone manual evidence.
