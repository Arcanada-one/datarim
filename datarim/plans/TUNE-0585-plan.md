...
blueprint_revision: 7e398df67df5d678e708c7bb483211fd7fccfacc
...
1. Resolve `approved_pair_commit` as the exact signed HEAD on which both preliminary subordinate reviewers returned YES; it must contain the exact plan blob under review and separately record the PRD blueprint revision from frontmatter. Using only the pre-existing governance key and `ssh-keygen`, create/sign the closed bootstrap-authorization object that pins the exact allowed bootstrap source/test paths plus non-empty role/skill/blueprint/constraint/policy/success-criterion arrays. Commit it alone as `bootstrap-authorization`. Add the bootstrap Bats contract and retained intended missing-executable output, run it RED at that assertion, and commit only test/evidence as `bootstrap-red`. Then implement only `scripts/customer-delivery-bootstrap.py`, make the bootstrap suite GREEN, and commit it as `bootstrap-source`. Dispatch a fresh subordinate exact-head compliance/quality review; preserve its report plus the closed bootstrap-review record and direct governance signature under `datarim-bootstrap-review-v1` in a separate `bootstrap-review` commit. The record binds exact authorization/report locators and digests, bootstrap commit/tree/executable/test/RED digests, finding digest, and verdict; later registration binds its locator/digest and signature locator/digest. Any finding returns to a new RED/source/review chain; no amend/rebase. Until the signed review passes, the bootstrap may not create keys, trust records, provider state, or general source.
   ...
   After the passing bootstrap review, create/sign `config/customer-delivery-bootstrap-provider-v1.json` binding the exact authorization/review/signature locators/digests, source commit/digest, Python/system executable versions/digests, fixed cwd/env, and exact request/result schema locators/digests for `canonicalize`, `caller-sign`, `genesis-plan`, `genesis-prepare`, `genesis-attest`, `genesis-bundle`, `authorize-and-sign`, `publish-blob`, `resolve-blob`, `lookup-provider-operation`, `append-receipt`, `lookup-receipt`, `resolve-receipt`, and `registry-cas`. Provision distinct registered non-repository `caller-sign-v1` key processes and private authority-signer identities for `registry-owner`, `spec-reviewer`, `atomicity-reviewer`, `evolution-owner`, `evolution-reviewer`, `quality-reviewer`, `evidence-runner`, `project-verifier`, `source-provider`, `deployment-provider`, `visual-provider`, `reconciliation-authority`, `anchor-authority`, and `release-authority`, plus the catalog's disjoint offline recovery quorum. Store each provider's unsigned, non-self-referential canonical registration payload and detached governance signature under `config/customer-delivery-provider-registrations/`; every payload retains the exact bootstrap registration/signature locators and digests, while registry records bind those four bootstrap fields plus separate provider-payload/signature locator/digest fields. Commit only bootstrap/provider registrations and signatures, public reciprocal caller/signer assignments, closed trust catalog, Datarim consumer registry in `genesis` mode, derived allowed-signers view, and detached governance signatures in a signed `trust-provision` commit descending from `bootstrap-review`. The registry binds governance fingerprint `SHA256:a3hhUtadjU+KNZ1zI2LQuS88vb+t7YnQUiA4IDNbLgM`, unique `(authority_provider_id, caller_subject) -> (caller_provider_id, signer_id)` mappings, each protected-ref provider's one `result_signer_id` and exact event/receipt-to-role tables, fixed signer provider/policy/caller plus request/decision namespaces, release authority namespaces `git` and `datarim-release-v1`, and permanent receipt authority `refs/heads/datarim/customer-delivery-receipts-v1` with repository/remote/ref, immutable bootstrap/provider payload/signature locator/digest tuples, exact GitHub ruleset ID/digest, deletion/non-fast-forward rules, and no bypass. Create/read back that branch/ruleset before any event attestation. In a disposable fixture, prove the caller-sign namespace/schema split, common publish/append/CAS control chain, response-loss generic lookup, and pointer-envelope registry-CAS wire verify the registry-owner signature in normal mode and that prior-catalog quorum recovery replaces compromised governance/registry-owner/anchor keys without consulting them, both with a prior anchor and under the PRD's exact enumerated null-anchor genesis field delta.
...
| Genesis | fourteen ordered commits before general source, including authorized/reviewed bootstrap TCB plus immutable spec/authorization/attestation requests; development-only raw RED, present-day signed mutation, protected-ref epoch | V-AC-9 |
...
| `config/customer-delivery-datarim-registry.json` | closed consumer schema, catalog link, reciprocal caller/signer/result-signer bindings, key/provider revocation, adoption checks |
...
2. Commit bootstrap authorization, bootstrap RED, bootstrap source, bootstrap review, trust provision, spec-review request, contract base, genesis freeze, authorization marker, authorization request, attestation plan, attestation requests, and authorization event in the exact declared order; run the concrete provider feasibility probe at its prescribed point.
