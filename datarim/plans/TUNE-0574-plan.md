...
After release, follow the current commands in `documentation/how-to/fleet-hook-sync.md`; do not invent a sync command. Update `/opt/datarim` and each per-user runtime on host-devs, dev-box, and client. Never touch `/home/client/code/client/local-stack/datarim`.
