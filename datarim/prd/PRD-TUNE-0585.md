...
Schema v1 also fixes reusable payload records. `authority` has exactly `registry_revision`, `role`, `key_id`, and `namespace`. It does not contain the event signature: the configured receipt provider resolves a detached signature by `(project_id, task_id, event_id, event_hash)`, preventing signature self-reference, and the validator verifies that receipt against the declared authority before accepting the event. `artifact_ref` has exactly `id`, `revision`, `sha256`, `locator`, and `binding_commit`. `receipt_ref` has exactly `profile`, `locator`, `sha256`, `signer_key_id`, and `namespace`; it refers to an independently existing provider record, never a signature over the event containing the reference. `typed_predicate` has exactly `kind`, `selector`, `operator`, and `expected`; `operator` is one of `equals`, `contains`, `matches`, `exists`, `not_exists`, `http_status`, `image_digest`, or `dom_fingerprint`, and `expected` is a JSON scalar or an ordered scalar array. Every payload contains `authority` plus exactly the following type-specific keys; every `[]` value is a duplicate-free ascending array unless explicitly described as ordered:
...
| `evolution_requested` | `request_id`, `artifact_class`, `artifact_before`, `proposed_action`, `owner_role`, `resume_cursor` |
...
| `prework_contract_frozen` | `contract_commit`, `requirement_graph_sha256`, `role_refs[]`, `skill_refs[]`, `blueprint_refs[]`, `constraint_refs[]`, `policy_refs[]`, `success_criterion_refs[]` |
| `implementation_authorized` | `repository_id`, `clean_base_sha`, `marker_commit`, `dirty_inventory_sha256` |
...
| `head_reconciled` | `conflict_event_ids[]`, `conflict_head_hashes[]`, `chosen_successor_event_id`, `reconciliation_receipt` |
| `event_superseded` | `target_event_id`, `target_event_type`, `replacement_event_id`, `reason`, `signer_scope`, `invalidation_set[]` |

The shipped `config/customer-delivery-event-v1.schema.json` is the executable transcription of this table. It may add regex, integer-range, enum, and cross-field restrictions, but may not add optional semantic fields or weaken cardinality. Every field ending `_receipt`, `_record`, or `_anchor`, plus `source_receipt`, `build_provenance`, and `customer_record`, is a `receipt_ref`; every field named `artifact_before`, `artifact_after`, or ending `_ref` is an `artifact_ref`, except `artifact_before` may be null only when `proposed_action` is `create`. `mapping_cardinality` has exactly non-negative integers `remark_count`, `requirement_count`, and `edge_count`; `resume_cursor`, counts, dimensions, byte lengths, and exit codes are signed 64-bit integers with the tighter non-negative constraints implied by their names. `predicate` is a `typed_predicate`. Enumerations are fixed: lanes are `customer-visible-v1`, `enabling-v1`, or `none`; requirement classes match the two armed lanes; atomicity/review verdicts are `pass` or `fail`; `proposed_action` is `create`, `revise`, `reuse`, or `no-change`; evolution dispositions are `reused`, `revised`, `created`, or `no-change`; customer dispositions are `accepted`, `returned`, `deferred`, `rejected`, or `withdrawn`; enabling disposition is only `verified`; legacy claim kinds are `implementation`, `test`, `screenshot`, or `deployment`. All referenced nested records must match the exact reusable records above. Any future payload field or event type requires a new schema version and migration, never permissive schema-v1 parsing.
...
Catalog schema v1 has exactly `schema_version`, `catalog_id`, `revision`, `valid_from`, `valid_until`, `canonicalization_profiles[]`, `receipt_profiles[]`, `minimum_profile_versions`, and `recovery_policy`. Consumer schema v1 has exactly `schema_version`, `registry_id`, `sequence`, `previous_registry_sha256`, `project_id`, `repository_id`, `mode`, `canon_epoch`, `framework_catalog_sha256`, `allowed_signers_sha256`, `verifiers[]`, `signers[]`, `providers[]`, `source_commitment_policy`, and `revocations[]`. Strict mode cannot revert. Every verifier pins platform, executable, version, executable SHA-256, algorithm, and profile. Every signer pins signer/key ID, fingerprint/public key, identity, exactly one authority role, permitted receipt types, namespaces, repository/project/task/requirement scopes, validity interval, predecessor key, and status. Every provider pins type, executable/version/digest, fixed cwd, argv schema, environment allowlist, timeout, output cap, result-schema digest, and authentication subject/audience. Every revocation pins monotonic sequence, previous digest, key ID, effective authority sequence, reason, and governance receipt. Unknown keys fail. The allowed-signers view digest must match the consumer registry and each line must reproduce one active signer without extra principals or namespaces.

Catalog and consumer signatures use distinct namespaces `datarim-trust-catalog-v1` and `datarim-consumer-registry-v1`. A key introduced by a consumer registry cannot authenticate that same registry. Genesis pins the governance SSH fingerprint `SHA256:a3hhUtadjU+KNZ1zI2LQuS88vb+t7YnQUiA4IDNbLgM` outside both artifacts in the reviewed bootstrap; verification first recomputes that fingerprint from the governance public key, verifies the catalog, then verifies the consumer registry against the catalog and predecessor. Ambient Git trust cannot replace this chain. Before freeze, TUNE-0585 provisions distinct private signer identities in the configured non-repository secret store and commits only public entries for `spec-reviewer`, `quality-reviewer`, `evidence-runner`, `project-verifier`, `source-provider`, `deployment-provider`, and `visual-provider`; governance and anchor authority remain pre-existing roots/providers. No key is reused across mutually exclusive roles, and the implementer/runner cannot hold the reviewer key in the review process. Customer keys are supplied only by a consumer registration and cannot be synthesized by the framework. Private keys, HMAC keys, provider credentials, and customer identities never enter Git.

Every receipt uses a canonical schema-v1 payload and detached signature. The common payload has exactly `schema_version`, `receipt_type`, `receipt_id`, `profile`, `registry_id`, `registry_sequence`, `registry_sha256`, `canon_epoch`, `project_id`, `repository_id`, `task_id`, `requirement_id`, `subject_sha256`, `subject`, `provider_sequence`, `issued_at`, and `idempotency_key`; fields that do not apply are explicit null, never omitted. The detached signature metadata fixes payload SHA-256, algorithm, namespace, signer/key ID, principal/issuer, and signature bytes. Receipt-type schemas close `subject` and bind: source manifests to head/count/exhaustion/order digest; atomicity review to source/requirement graph/cardinality/predecessor/spec/verdict; quality review to exact commit/tree/ledger blob/head/count/evidence/finding digest/verdict; RED/GREEN to assertion/argv/tree/mutation/output/retained evidence; project verification to requirement and evidence receipts; customer disposition to stable authority record/requirement/deployed SHA/private commitment; deployment/visual evidence to exact observation; and anchor receipts to task/project/event head/count/blob OID/length/provider ref/commit/previous receipt. Distinct fixed namespaces apply to every receipt class. Wrong class, role, namespace, scope, registry revision, or subject binding fails even under a valid key.
...
4. Once the runner exists, a present-day controlled mutation produces canonical signed RED, the restored exact head produces signed GREEN, and all final ledger/evidence changes are committed. An independent quality reviewer then signs an external exact-head receipt and compare-appends it to the protected receipt authority; the implementation head does not change after the review.
5. The bootstrap remains `genesis_unanchored` and cannot report canonical completion until that exact reviewed head is merged, the signed release tag's annotation binds the full merged commit/tree and ledger blob/event head/count, and an independent verifier refetches the tag/receipt and ledger bytes through `git-immutable-tag-v1`. The verified external tag receipt becomes the `canon_epoch`; it is sufficient without a recursive in-ledger or next-release record. All later tasks use the normal strict sequence with a configured `git-protected-ref-v1` branch.
...
Migration is byte-deterministic and split into plan/apply. Planning is `migrate --plan --mode genesis|legacy-v0 --task <id> --project <id> --input <bootstrap-or-mapping-json> --source-snapshot <signed-manifest-json> --authorization-commit <full-sha-or-null> --canon-epoch <version@full-sha-or-null> --compatibility-release <version-or-null> --removal-version <version-or-null> --recorded-at <UTC-seconds> --output <migration-plan-json>`. Apply is `migrate --apply --plan <migration-plan-json> --plan-sha256 <lowercase-hex> --output <ledger-jsonl>`. All paths are project-root-confined regular files. The signed source snapshot includes exhausted pagination cursor, immutable anchor receipt, stable provider order, and every source commitment. The input schema lists every stable source item, ordered atomic requirement/AC, explicit legacy claim, and, for genesis, frozen pre-work/authorization bindings. The caller supplies time; migration never reads the clock or ambient network.
...
Planning writes no ledger. Apply rejects source/input drift, recomputes the plan and intended bytes, builds and validates complete bytes in a root-confined temporary sibling, fsyncs them, refuses an existing non-identical target, treats an identical target as idempotent success, atomically renames, and fsyncs the directory. Provider timeout, pagination gap, plan-digest mismatch, ID collision with different semantics, invalid mapping, partial write, or validation failure leaves the target unchanged and emits no passing receipt. Replaying identical inputs produces byte-identical plan and ledger bytes. Migration never anchors automatically; after validation it remains `unanchored` until the normal provider publish/resolve gate succeeds. Only then may the external consumer registry compare-and-swap from `legacy` to the one-release dual-read state and later to irreversible `strict`; a crash after anchor but before registry CAS resumes from anchor resolution without remigration.
...
TUNE-0585's reviewed spec, genesis freeze, and authorization are three ordered signed commits before implementation; raw TDD is not canonical evidence; present-day signed mutation RED/restored GREEN, focused/full exact-head validation, English-only surface, exact-blob spec-review disposition, external non-recursive exact-head quality-review receipt, and post-merge refetch/verification of the signed tag receipt plus full commit/tree/ledger blob/event head/count are independently verified.
Covers: D-REQ-12
