...
A migration plan has exactly `schema_version:uint=1`, `profile:migration-v1-literal`, `mode`, `project_id:id128`, `task_id:id128`, `input_sha256:sha256`, `source_snapshot_sha256:sha256`, `registry_sha256:sha256`, `registry_sequence:uint`, `authorization_commit:git_oid|null`, `canon_epoch` (consumer epoch shape or null), `compatibility_release:text|null`, `removal_version:text|null`, `recorded_at:utc_seconds`, `event_signer:id128`, `receipt_output:relative_path`, `provider_sequence:uint`, `ordered_input_sha256:sha256`, `events[]`, `event_count:uint`, `ledger_byte_length:uint`, `ledger_sha256:sha256`, `ledger_head:sha256`, `coverage_classification:id128`, and `findings[]`. Each plan event has exactly `semantic_source_id:id128`, `event` (one complete closed schema-v1 event object including `event_hash`), `event_attestation_id:id128`, and `receipt_template`. The template is the complete common `event-attestation` receipt payload with `provider_sequence:null`; `issued_at` equals plan `recorded_at`; `receipt_id` is `mig-receipt:` plus the event ID digest; and `idempotency_key` is SHA-256 of canonical JSON containing exactly `profile`, `ordered_input_sha256`, `event_index`, `event_id`, and `event_hash`. Each finding has exactly `code:id128`, `subject_id:id128`, and `message:text`. The concatenation of canonical `event` objects plus one LF each must exactly match the planned ledger byte length/SHA/head, so apply can reconstruct the ledger from the plan while independently comparing supplied source/input/registry digests. Unknown keys fail.
...
The recovery manifest has exactly `schema_version:uint=1`, `plan_sha256:sha256`, `ledger_path:relative_path`, `ledger_sha256:sha256`, `transition_index:uint`, `phase`, `receipt_states[]`, `target_installed:boolean`, `anchor_receipt:receipt_ref|null`, `registry_before_sha256:sha256`, `registry_after_sha256:sha256|null`, and `recovery_receipt:receipt_ref`. Phase is `planned`, `receipts-reserved`, `receipts-published`, `target-installed`, `anchored`, or `registry-committed` and advances only in that order. Each receipt state has exactly `event_id:id128`, `idempotency_key:sha256`, `reserved_sequence:uint`, `reservation_token_sha256:sha256`, `payload_sha256:sha256|null`, `receipt_locator:locator|null`, `receipt_sha256:sha256|null`, and `state` (`reserved`, `published`, `refetched`, or `bound`). It is sorted by event index, atomically rewritten/fsynced, and every transition is authenticated by a compare-appended `migration-recovery` receipt. If remote publication precedes the local update, recovery queries the authority by the plan's deterministic idempotency keys and reconstructs the exact next manifest; local state is never sole authority.

Planning writes no ledger, reservation, or event receipt. Apply rejects source/input/registry drift and recomputes plan/intended bytes. For each receipt template it invokes `reserve-receipt-sequence` with expected provider head and deterministic idempotency key; the authority atomically returns a signed reservation with a stable sequence/token, and any retry of that key returns the identical reservation even after concurrent appends. Apply replaces only template `provider_sequence:null` with the reserved sequence, verifies `issued_at` and idempotency are unchanged, signs the canonical payload, compare-appends it against the reservation token, and refetches/verifies the tuple before target installation. Missing/wrong-role attestations fail even in `unanchored` genesis. Apply then builds and validates complete ledger bytes in a root-confined temporary sibling, fsyncs them, refuses an existing non-identical target, treats an identical target/receipts as idempotent success, atomically renames, and fsyncs the directory. Provider timeout, pagination gap, plan-digest mismatch, ID collision with different semantics, invalid mapping, receipt failure, partial write, or validation failure leaves the target unchanged. Because attestations are append-only, failure after publication leaves them `pending-orphan`; they are not passing evidence and cannot authorize a stage until an identical retry installs/validates the named ledger and advances their recovery states to `bound`. Replaying identical inputs produces byte-identical plan/ledger bytes, idempotency keys, reservations, and receipt payloads. Migration never anchors the ledger head automatically; after event validation it remains `unanchored` until normal provider publish/resolve succeeds.

Only after anchor resolution may the external consumer-registry owner invoke `registry-cas`. Its request has exactly `schema_version:uint=1`, `operation:registry-cas-literal`, `idempotency_key:sha256`, `expected_registry_sequence:uint`, `expected_registry_sha256:sha256`, `replacement_registry_locator:locator`, `replacement_registry_sha256:sha256`, `replacement_signature_locator:locator`, and `anchor_receipt:receipt_ref`; its authenticated result has exactly `schema_version:uint=1`, `idempotency_key:sha256`, `previous_registry_sequence:uint`, `previous_registry_sha256:sha256`, `result_registry_sequence:uint`, `result_registry_sha256:sha256`, `provider_sequence:uint`, `result_locator:locator`, and `result_sha256:sha256`. The provider compares expected sequence/digest, verifies governance signature/predecessor/anchor, appends exactly once, and refetches the successor. It transitions `legacy` to one-release dual-read and later irreversible strict, or `genesis` to the verified non-null `canon_epoch`. A crash after anchor or registry publication resumes from anchor/registry resolution without remigration.

For recovery authentication, `receipt_state_sha256` hashes the canonical recovery manifest with `recovery_receipt` replaced by null; after compare-append/refetch, the local view installs that receipt reference. This removes self-reference while binding every other byte.
...
Exact `canon_epoch`, one-release dual read, strict-mode non-reversion, genuine-source import, and `legacy_claim_imported` non-coverage fixtures pass independently. Ambient-clock/unregistered-network access, byte/order drift, missing or wrong-role event attestations, sequence-reservation collision/idempotency drift, changed source/registry between plan/apply, existing-identical versus existing-different target, plan replay non-identity, invalid recovery phase/digest/signature, remote-published/local-missing recovery, temp/short-write/fsync failure, anchor-before-registry-CAS inversion, crash-after-anchor resume, and registry-CAS request/result stale-sequence/digest mutants each fail at their named boundary without changing an acknowledged target.
Covers: D-REQ-09
...
TUNE-0585's exact reviewed PRD/plan pair, trust provision, contract base, genesis freeze, authorization marker, and authorization event are six ordered signed commits before implementation; raw TDD is not canonical evidence; present-day signed mutation RED/restored GREEN, focused/full exact-head validation, English-only surface, exact-blob spec-review disposition, external non-recursive exact-head quality-review receipt, embedded Git-namespace tag signature, separate `datarim-release-v1` receipt, registry-successor CAS/refetch, and post-merge verification of current host rules plus full tag/commit/tree/ledger blob/event head/count are independently verified.
Covers: D-REQ-12
