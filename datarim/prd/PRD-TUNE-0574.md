...
- Touching Client’s gated fork, dependabot PRs, or unrelated salvage directories.
...
- **Fleet readback**: after merge, the installed runtime on arcana‑devs, dev‑ai, and client is synchronised and drift gates re‑run, producing identical SHA or content verification; exact‑main proof (no divergent changes on `origin/main` at merge time).
...
After the PR is merged and the release cut, on each of the three machines (arcana‑devs, dev‑ai, client):
...
**How to verify**
- Execute the exact grep from the updated doc on arcana‑devs, dev‑ai, client; all return a line containing the marker.
