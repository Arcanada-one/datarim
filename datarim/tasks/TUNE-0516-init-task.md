...
Symptom this fixes (generalized, NO Arcanada/DEVS/Client/tailscale/space names in shipped text): an agent acting as orchestrator repeatedly PAUSES to ask the operator "where do I run this / will I collide with the other running session?" and tends to SERIALIZE tasks, when the correct default is: each task gets its own isolated orchestrator session (own working tree + own branch), and parallel sessions are the expected norm — isolated by branch and by file set. No question is warranted unless two tasks genuinely touch the SAME branch or SAME files.
...
=== HARD CONSTRAINTS (shipped-surface hygiene) ===
  ...
  - Zero operator-specific names: no Arcanada, DEVS, dev-box, Client, tailscale, space.yml literals, no real hostnames/IPs. All examples use placeholders and are marked non-normative.
