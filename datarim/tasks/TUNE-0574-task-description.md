...
7. **Fleet consistency**
   After merge, `/opt/datarim` (and per‑user installs) on arcana‑devs, dev‑ai, client are synchronised. Drift gates re‑run and all three machines show repo == installed runtime.
...
- Commit as `Arcanada <dev@veritasarcana.ai>`. Never as Arcanada.
- Branch + PR; rebase on `origin/main` immediately before merge. Never rebase or force‑push shared main.
- Do NOT touch `/home/client/code/client/local-stack/datarim` (Client’s gated fork).
