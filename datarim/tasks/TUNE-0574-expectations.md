...
- **6. The semantic hook probe and released runtime are verified on all three machines.**
  ...
  - Что хочу проверить: host-devs, dev-box, and client carry the released framework and per-user runtime, pass drift/readback checks, and return the semantic marker from the installed hook; the gated Client fork remains untouched.
