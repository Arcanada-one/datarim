...
@test "tmux attach valid pane prints russian instructions + tmux_cmd" {
    _start_mock tmux_attach_200
    run "$DATARIM_BIN" tmux attach %0
    [ "$status" -eq 0 ]
    [[ "$output" == *"tmux attach-session"* ]]
    [[ "$output" == *"%0"* ]]
}

@test "tmux attach --task <TASK-ID> accepts task arg" {
    _start_mock tmux_attach_200
    run "$DATARIM_BIN" tmux attach %0 --task TUNE-9999
    [ "$status" -eq 0 ]
}

@test "V-AC-22: tmux attach --json response carries data.tmux_cmd (machine-parsable)" {
    _start_mock tmux_attach_200
    run "$DATARIM_BIN" tmux attach %0 --json
    [ "$status" -eq 0 ]
    printf '%s' "$output" | python3 -c "import json,sys; d=json.load(sys.stdin); assert 'tmux_cmd' in d['data']; assert d['data']['tmux_cmd'].startswith('tmux attach-session')"
}

@test "V-AC-21: tmux attach with malformed pane id → exit 31 NOT_FOUND" {
    run "$DATARIM_BIN" tmux attach ../../etc/passwd
    [ "$status" -eq 31 ]
}
