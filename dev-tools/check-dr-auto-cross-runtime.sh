...
#      at datarim/.auto-mode-active. (Question Suppression Ladder engages off
#      this marker, not off the runtime identity.)
...
[ -f "$MARKER_TOOL" ] || { echo "ERROR: missing $MARKER_TOOL" >&2; exit 2; }
[ -f "$FB_RULES" ]    || { echo "ERROR: missing $FB_RULES" >&2; exit 2; }
...
if bash "$MARKER_TOOL" reassert --root "$WORK" --task-id "$SMOKE_TASK_ID" \
        >/dev/null 2>&1 \
    && grep -q "task_id: ${SMOKE_TASK_ID}$" "$WORK/datarim/.auto-mode-active" 2>/dev/null; then
    log "check 1 (activation marker) ...... PASS"
else
    note_fail "check 1: reassert did not write a parseable marker for ${SMOKE_TASK_ID}"
fi
