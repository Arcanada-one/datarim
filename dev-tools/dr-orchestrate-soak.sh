...
# Deploy:
#   scp dev-tools/dr-orchestrate-soak.sh root@host-dev:/usr/local/bin/
#   ssh root@host-dev 'chmod 0755 /usr/local/bin/dr-orchestrate-soak.sh'
# Launch (detached):
#   ssh root@host-dev 'setsid /usr/local/bin/dr-orchestrate-soak.sh \
#       >>/var/log/dr-orchestrate-soak.log 2>&1 </dev/null & disown'
# Verdict after window:
#   ssh root@host-dev '/opt/datarim/dev-tools/measure-orchestrator-soak.sh \
