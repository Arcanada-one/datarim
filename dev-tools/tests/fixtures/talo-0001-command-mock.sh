...
case "$command_name" in
    ...
    gh)
        ...
        case "$*" in
            ...
            *"runner-groups/42/runners"*)
                ...
                runner='{"id":7001,"name":"talo-0001-trusted-host-devs","os":"Linux","status":"offline","busy":false,"labels":[{"name":"self-hosted"},{"name":"Linux"},{"name":"X64"},{"name":"talo-0001-trusted"}]}'
    ...
    sudo)
        ...
        if [ "${TALO_MOCK_REAL_UID:-0}" = 1 ] && [ -n "$sudo_user" ]; then
            /usr/bin/sudo \
                --preserve-env=ACTIONS_RUNNER_INPUT_TOKEN,TALO_MOCK_LOG,TALO_MOCK_CONFIG_CMDLINE,TALO_MOCK_CONFIG_ENV_REMOVED,TALO_MOCK_CONFIG_ENV_LEAK,TALO_MOCK_CONFIG_STARTED,TALO_MOCK_CONFIG_REMOTE_ONLY,TALO_MOCK_CREDENTIALS_TEMPLATE,TALO_MOCK_RSA_TEMPLATE,TALO_MOCK_CONFIG_FAILURE,TALO_MOCK_REPLACE_EXEC \
                -u "$sudo_user" -- "$@"
        else
            "$@"
        fi
