...
@test "wiring: the Rename Policy prescribes the tool, and CI runs this suite" {
    # Two wiring points, both anti-decay:
    #  1. the instruction surface that states the rename policy must point at the tool,
    #     so an agent asked to rename a prefix finds it instead of hand-rolling sed;
    #  2. dev-tools/tests/*.bats is NOT swept by `bats tests/` — the CI job enumerates
    #     files, so an unlisted suite runs nowhere at all.
    ROOT="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)"
    SKILL="$ROOT/skills/datarim-system/task-identity-and-context.md"
    [ -f "$SKILL" ]
    run grep -c 'rename-task-prefix.sh' "$SKILL"
    [ "$status" -eq 0 ]
    run grep -c 'MUST use `dev-tools/rename-task-prefix.sh`' "$SKILL"
    [ "$status" -eq 0 ]
    WF="$ROOT/.github/workflows/dev-tools-lint.yml"
    run grep -c 'bats dev-tools/tests/rename-task-prefix.bats' "$WF"
    [ "$status" -eq 0 ]
}
