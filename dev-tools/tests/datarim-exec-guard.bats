...
bindings:
  - workspace: $ARCANADA_WS
    space: arcanada
    required_host: host-devs
    host_aliases: [host-devs, Host-DEVS]
  ...
  - workspace: $CLIENT_WS
    space: client
    required_host: dev-box
    host_aliases: [dev-box]
...
# ---------------------------------------------------------------------------
@test "deny-arcanada: executable /dr-* Bash call in bound Arcanada workspace denies with host-devs directive" {
    run bash -c "
...
# ---------------------------------------------------------------------------
# V-AC-2 / D-REQ-03, D-REQ-01: deny-client
# ---------------------------------------------------------------------------
@test "deny-client: executable /dr-* Bash call in bound Client workspace denies with dev-box directive" {
    run bash -c "
      fixture_bash() { local cwd=\"\$1\" cmd=\"\$2\"; jq -nc --arg cwd \"\$cwd\" --arg cmd \"\$cmd\" '{hook_event_name: \"PreToolUse\", tool_name: \"Bash\", tool_input: {command: \$cmd}, cwd: \$cwd}'; }
      fixture_bash '$CLIENT_WS' 'claude /dr-plan SPACE-1234' | DATARIM_EXEC_GUARD_MAP='$MAP' DATARIM_EXEC_GUARD_AUDIT_LOG='$AUDIT_LOG' bash '$GUARD'
    "
    [ "$status" -eq 0 ]
    [[ "$output" == *"permissionDecision"*"deny"* ]]
    [[ "$output" == *"dev-box"* ]]
