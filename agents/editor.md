...
- **Social link/button language tags**: every cross-link in a first comment and every button in the article `social` back-link block must show the destination content language (`X (EN)`, `Telegram (RU)`, `LinkedIn (EN)`, `Facebook (RU)`, blog `(EN)`/`(RU)`). Flag any untagged social link as a blocker. Also verify a cross-posted blog article carries the `social` back-link block at all — its absence is an incomplete publish. See `publishing.md` § Universal rule.
