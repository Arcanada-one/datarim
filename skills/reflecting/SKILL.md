...
6.  **EVOLUTION**:
    ...
    - **Apply-target resolution (MANDATORY before write):** resolve where an approved proposal lands per `$HOME/.claude/skills/evolution/SKILL.md` § Apply-Target Resolution (global vs project-local). The write target is relative to `${DATARIM_RUNTIME:-$HOME/.claude}`, NOT hard-coded to `$HOME/.claude/`. When `/dr-do` runs in a project-local (`install.sh --project`) context — `DATARIM_RUNTIME` ends in `/.datarim`, or a `.datarim/` runtime dir is found walking up from cwd — a **project-specific** lesson writes to `DIR/.datarim/skills/…`; an **ecosystem-general** lesson is surfaced for promotion to the shared runtime rather than silently written there. In a global/symlink install both scopes resolve to the shared runtime.
    - **Stack-agnostic gate (MANDATORY before write to a SHARED runtime target):** before applying any approved Class A proposal that writes to the shared `${DATARIM_RUNTIME:-$HOME/.claude}/{skills,agents,commands,templates}/`, load `$HOME/.claude/skills/evolution/stack-agnostic-gate.md` and run the gate against the proposal text (script form: `scripts/stack-agnostic-gate.sh <target>`). FAIL → reject the proposal, do NOT write; reword to stack-neutral or escalate to user as «belongs in project's CLAUDE.md, not framework runtime». A project-local `DIR/.datarim/` target MAY carry project-specific (stack-bound) content and is exempt from this gate (it is not shipped to other consumers).
