...
5.  **ACTION**:
    ...
    - **Known-fix writer:** decide whether the completed task produced a verified, reusable failure-to-fix mapping. If it did, append exactly one fenced ` ```json known_fix ` object to the existing `datarim/insights/INSIGHTS-{task-id}.md` with only these fields: `schema_version` (`1`), `task_id`, `failure_class` (lowercase hyphenated slug), non-empty arrays `symptoms`, `fix_steps`, `verification`, and `source_refs`, plus `root_cause` and confidence (`low`, `medium`, or `high`). Cite only relative Markdown paths. Run `${DATARIM_RUNTIME:-$HOME/.claude}/dev-tools/known-fix-memory.py validate --root "$DATARIM_ROOT" --task {task-id}` and fix any rejection before returning. If there is no verified reusable fix, write `Known Fix: none` in the reflection and do not invent a record. Never include credentials or merely recalled/unverified claims.
6.  **EVOLUTION**:
