...
**Mandatory reverse link — the article page MUST link back to every social post (bidirectional contract).** Cross-posting is not finished when the social posts are live. The blog article itself MUST carry a "discuss on social" block linking to **every** social post created for it (Telegram + X + LinkedIn + Facebook), in **both** language versions of the page. The social→blog link (first comment) and the blog→social links (this block) are two halves of one contract — shipping only the first half is an incomplete task, not a partial pass.

On the Arcanada landing (`Projects/Arcanada Ecosystem/code/landing`) the mechanism is canonical and per-article: each post file `pages/blog/posts/<slug>.php` declares a top-level `'social' => [...]` map, which `pages/blog/show.php` renders as the localized "discuss on social" block (`t('discuss_on_social')`). The map keys and the exact permalink shapes (verified 2026-06-21):

```php
'social' => [
    'telegram' => 'https://t.me/operatortypes/<msg-id>',
    'x'        => 'https://x.com/VeritasArcanaAI/status/<status-id>',
    'linkedin' => 'https://www.linkedin.com/feed/update/urn:li:activity:<activity-id>/',
    'facebook' => 'https://www.facebook.com/arcanada/posts/<pfbid…>',
],
```

The block is **language-agnostic** — declared once at the top of the post array (not under `ru`/`en`), so both `/ru/blog/<slug>` and `/en/blog/<slug>` render it. Capture each permalink at publish time (X gives the status URL on the post page; FB via the post's overflow `...` menu -> copy-link -> `pfbid...`; LinkedIn from the post's timestamp link -> `urn:li:activity:...`; TG is `t.me/<channel>/<msg-id>`). After editing the post file, re-deploy (`deploy.sh <domain>`) and **verify the live page actually renders all four links** — `curl`/WebFetch the rendered `/ru/` and `/en/` URLs and grep for the social hosts, not just HTTP 200. A published-true article with no `'social'` block is an unfinished cross-post — at QA it is a missed acceptance criterion (the task's `social-блок 4 URL` line), routed back to `/dr-do`, NOT a pass-with-notes. <!-- allow-non-ascii: literal-russian-acceptance-criterion-token-cited-verbatim -->
...
**Telegram channel-post layout for a blog article (canonical, 2026-06-20).** A blog-article announcement on a TG channel is **exactly two top-level posts, no comment**. Photo+audio cannot be combined into one post (sendMediaGroup does not mix media types, and a `reply_to_message_id` against a channel post does NOT nest — it renders as a separate top-level post). So use a **video** to merge cover image and narration into a single card:

1. **Post 1 — video (cover image + article narration) + intro text + «read more».** Build an mp4 from the article hero image (static) plus the narration mp3 (`ffmpeg -loop 1 -i cover.jpg -i narration.mp3 -c:v libx264 -tune stillimage -pix_fmt yuv420p -vf scale=trunc(iw/2)*2:trunc(ih/2)*2 -c:a aac -shortest out.mp4`), send via `sendVideo`. The caption (≤1024 UTF-16 units) carries the article title and an intro paragraph that ends pointing the reader to the full text below — NOT the whole article.
2. **Post 2 — the long text + the site link at the end.** The full article gist (what the products do, the honest roadmap caveat — real substance, not a thinned-out bullet list) goes here as a plain `sendMessage` (≤4096 units), with a single hidden link to the article as the closing line.

**The site link goes in exactly one of the two posts — the long-text post (Post 2), at the end — never in the video caption.** Duplicating the link in both posts is redundant.

**No promotional copy about the narration.** Do NOT describe which TTS voice was used, do NOT add «this article is narrated by …» marketing lines, do NOT explain the synthesis engine. The narration is simply the video's audio track; the reader does not care how it was produced.

**Hidden links in Telegram (HTML parse_mode) — preferred over bare URLs.** TG supports `<a href="URL">anchor text</a>` with `parse_mode=HTML`. Use it so the reader sees only blue anchor text, not a raw `https://…` string — it is cleaner. Example: `<a href="https://arcanada.ai/ru/blog/slug">Читать статью целиком</a>` instead of `Статья целиком: https://arcanada.ai/ru/blog/slug`. Apply the same to any in-caption link. (This is TG-specific: FB/LinkedIn/X plain-text platforms cannot hide a URL behind anchor text via the API.)
...
1. **Use the inline post composer, not the standalone media editor.** On both LinkedIn and Facebook, open the **inline post composer** (LinkedIn «Start a post» / FB «Создать публикацию») — the dialog with the "What do you want to talk about?" / "Что у вас нового" text area. Paste the image *into that composer window* (see #2). Do NOT route through the standalone media **Editor** («Select files to begin» / «Upload from computer») reached by clicking Photo on the cold feed — that surface only takes the OS file picker (invisible to automation) and ignores a clipboard paste. The inline composer accepts a pasted image whether the text is written first or the image is pasted into the empty composer first — order does not matter. <!-- allow-non-ascii: literal-platform-ui-button-labels-cited-verbatim -->

2. **Attach the media yourself — never hand the file picker to the operator while an automated path exists.** The agent owns the full publish, media included; making the operator browse to the file on disk is a regression, not a workaround. Per-platform capability, try in order and only escalate when the platform genuinely blocks every automated path:
   - **X / Twitter — fully automatable, no operator.** Clipboard-paste of an image works (`<file> | pbcopy` of the image, then click the composer media affordance / `Cmd+V` into the composer), and `file_upload` against the composer's file input accepts both images and **video** (the A4 / A5 cover-video posts went in this way). Drive the upload, wait for the upload-complete state (the localized "Ready" label), snapshot the preview, then set text. Do NOT ask the operator to pick the file.
   - **Facebook — clipboard image paste IS accepted (verified 2026-06-20).** Do NOT open the OS file picker and do NOT use `file_upload` (the tool no longer reads host paths). Instead load the image *bitmap* onto the macOS clipboard with `osascript -e 'set the clipboard to (read (POSIX file "/abs/cover.jpg") as JPEG picture)'`, click into the composer text area, and `Cmd+V` — FB attaches it as a media preview. Stage the post TEXT first (its own clipboard paste), then swap the clipboard to the image bitmap and paste again. **Always read the clipboard back (`pbpaste`) immediately before each `Cmd+V`** — a background process can overwrite it between `pbcopy` and the paste (observed: a stray `/dr-auto …` string landed in the FB comment field once); if the readback is wrong, re-copy and re-verify before pasting.
   - **LinkedIn — clipboard image paste IS accepted in the inline post composer (verified by operator 2026-06-21).** Plain `Cmd+V` of an image bitmap into the «Start a post» composer window attaches the image directly — exactly like Facebook, no OS picker, no `file_upload`. Operator-confirmed sequence: open «Start a post» → put the image bitmap on the clipboard (`osascript -e 'set the clipboard to (read (POSIX file "/abs/cover.jpg") as JPEG picture)'`, or a `Cmd+C` on the file in Finder) → click into the composer window → `Cmd+V` → the image appears in the composer → then write/paste the text. Image-first into the empty composer works; text-first then image also works. Do NOT click the standalone «Select files to begin» / «Upload from computer» Editor — that path only takes the invisible OS picker and ignores the paste (this was the surface that misled an earlier session into wrongly reporting "LinkedIn paste does not work"). `file_upload` is disabled (no host-path reads) and cross-origin `fetch` of the cover is CSP-blocked, so the clipboard paste is THE path. As on FB, `pbpaste`-verify the clipboard right before each `Cmd+V`. <!-- allow-non-ascii: literal-platform-ui-button-labels-cited-verbatim -->

   Default posture for every platform: **the agent uploads the media autonomously via the macOS image clipboard** — X, Facebook, and LinkedIn are ALL automatable through a clipboard paste into the inline composer; no operator, no OS file dialog. Never click «Upload from computer» / open an OS picker as a first move on any platform. Operator file-picking is a last resort only, after a clipboard paste into the inline composer has actually failed on that specific platform. <!-- allow-non-ascii: literal-platform-ui-button-labels-cited-verbatim -->
...
- [ ] No "Client" named in public content — replace with "primary work" or
