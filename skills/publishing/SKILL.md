...
- [ ] Not written via an external writing assistant — voice-bearing content
  is generated by the assigned model, not delegated. The coworker hook guard
  (`dev-tools/coworker-hook-guard.sh`) blocks `coworker write` to voice-bearing
  paths pre-hoc. If this item fails (coworker was used despite the gate),
  flag as a hook bypass, rewrite natively, and report the bypass vector
  for the hook's exclusion list.
