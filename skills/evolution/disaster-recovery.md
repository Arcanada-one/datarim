...
1. **Grep all reflection docs by filename** — not just reflections of the "obvious parent" task. Search every `datarim/reflection/*.md` across all projects in the incident window for any mention of the lost filename. A reflection from an unrelated task may have proposed changes to the file.
...
A prior archive claimed 4 files "text reconstruction not possible" after 0 minutes of discovery. A follow-up recovery pass restored 100% of them in 20 minutes using channels 1-3. The cost of this checklist is ~25 minutes; the cost of a false "loss" claim is permanent content gap plus eroded trust in archive accuracy.