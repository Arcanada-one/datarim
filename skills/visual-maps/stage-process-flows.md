...
Every stage flow above ends with the agent emitting a canonical CTA block ([definition](../cta-format/SKILL.md)) per `${DATARIM_RUNTIME:?}/skills/cta-format/SKILL.md`. The CTA is part of the response — not a separate stage. Mapping of stage end → primary CTA command:
