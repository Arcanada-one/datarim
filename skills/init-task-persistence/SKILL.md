...
For example, an L3 epic may legitimately require several rounds across
multiple `/dr-do` sessions; reflection should assess whether that round count
matches the planned decomposition instead of treating it as automatic churn.
...
ISO timestamp of the originating append-log entry. A compliant `/dr-do`
round inherits the
