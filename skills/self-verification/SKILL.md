...
The automatic L2+ order is **deterministic floor → guarded auto-fix loop → model reviewer dispatch**. The floor remains read-only. After its canonical findings pass parent validation, the parent may invoke `dev-tools/self-verify-auto-fix.sh` for only the registered formatting, trailing-whitespace lint, and exact-typo tuples. Manual `/dr-verify` never enters this loop.
...
**Two profiles are canonical.** Manual `/dr-verify` retains full tri-layer, findings-only verification. Successful `/dr-prd`, `/dr-plan`, and `/dr-do` stages use the one-pass `post_step` profile with L1 off, one L2 reviewer, or three parallel L3/L4 roles; L2+ may run the guarded deterministic auto-fix loop before reviewer dispatch.
