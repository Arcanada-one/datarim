...
Source: DEV-1196 reflection §2.4 — 10 phases (model upgrade → knowledge base → evolution skill → retrieval hook → installer → sync cron → digest cron → auto-promotion gate → Client-internal contract → pre-merge rehearsal) formed a closed loop; one-MR delivery with 4 design docs released before code was the right strategy. Per-phase MRs would have shipped 10 broken intermediate states.
