...
- host-dev has 62 Gi RAM (self-imposed laptop constraint)
...
INFRA-0020 installed torch + sentence-transformers + 30 deps into system Python on host-db. Future `pip install` for another service may upgrade a shared dependency and break the embedding API silently. The same pattern occurred in LTM-0002 (Docker was the correct isolation) and EMAIL-0001 (system pip, no isolation).
