...
source: recurring task-ID collision reflection
---
...
**Use `${DATARIM_RUNTIME:?}/dev-tools/next-free-id.sh <PREFIX> <DATARIM_ROOT>`.
It is the only
...
```sh
NEW_ID="$("${DATARIM_RUNTIME:?}/dev-tools/next-free-id.sh" TUNE "$DATARIM_ROOT")"
```
...
2. **Explicit release** — `${DATARIM_RUNTIME:?}/dev-tools/next-free-id.sh --release <ID> <ROOT>`.
...
1. **Choose new ID.** Allocate with `${DATARIM_RUNTIME:?}/dev-tools/next-free-id.sh <PREFIX> <ROOT>`
...
A resumed task once hit this exact collision during `/dr-archive`: a parallel
session had reserved and committed the same identifier first. Resolution
required renaming the whole derived chain mid-archive, which is why this skill
codifies the procedure.
