...
## Datarim integration profile

Within Datarim, directory-scoped `CONTRACTS` are persistent implementation invariants, not a
replacement for PRD D-REQs, V-ACs, plans, tests, or QA verdicts. Before editing a subtree, read the
nearest `CONTRACTS` plus every ancestor `CONTRACTS` up to the repository root. If a task conflicts
