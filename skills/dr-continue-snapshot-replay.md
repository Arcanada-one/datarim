---
name: dr-continue-snapshot-replay
description: Consumer contract for stage snapshots — /dr-continue and /dr-orchestrate read datarim/snapshots/{TASK-ID}.snapshot.md first, emit replay-prompt with CTA + bilingual autonomy reminder + `done before:` body.
...
`/dr-continue` и `/dr-orchestrate` после Task Resolution читают снапшот **первым** — до task-description, init-task, activeContext, PRD, plan. Если снапшот существует и валиден — собирают replay-промпт по канонической форме и продолжают автономно. Если снапшота нет или он malformed — fallback на legacy-поведение без warning-шума.

## Contract

| Aspect | Value |
|--------|-------|
| Consumer touchpoints | `commands/dr-continue.md` § Step 2.5, `plugins/dr-orchestrate/commands/dr-orchestrate.md` § Snapshot-First Resume |
...
- `commands/dr-continue.md` § Step 2.5 — consumer-точка
