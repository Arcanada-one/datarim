...
- Multi-datasource projects where more than one client / connection / schema exists and a specific call must
  target a specific one (e.g. reads from `stats` vs `analytics_db`, from `primary` vs `replica`, from tenant A
  vs tenant B).
...
Reference incident: **DEV-1156** (app-v2). A raw query intended to hit `stats` (mysql5) was injected on the
`analytics_db` client (mysql8). Unit tests mocked the Prisma client and passed green. Production returned
...
Reference incident: **DEV-1183** (app-py-scripts code-review bot). `claude -p` exits `0` when the 5-hour
