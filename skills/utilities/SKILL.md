...
- `datarim-sync.md`
  Use for synchronizing framework files between `${DATARIM_RUNTIME:?}/` and the Datarim repo.
- `ga4-admin.md`
...
- `shell-conventions.md`
  Use when writing or reviewing a shell helper that returns lists, iterates over them, splits on a delimiter, or runs a regex. Canonical IFS / word-splitting / locale rules (newline-separated `printf '%s\n'` returns, `while IFS= read -r` loops, narrow `IFS` scoping, `LC_ALL=C` for regex/sort) for cross-platform (macOS bash 3.2 / Linux) behaviour. Runnable skeleton: `${DATARIM_RUNTIME:?}/templates/shell-helper-template.sh`.
