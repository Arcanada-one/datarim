...
> **Usage:** Load when writing or reviewing a Datarim shell helper that returns lists, iterates over them, splits on a delimiter, or runs a regex. These four conventions keep behaviour identical across macOS (bash 3.2, non-POSIX collation) and Linux (bash 5.x). Runnable skeleton: `${DATARIM_RUNTIME:?}/templates/shell-helper-template.sh`.
