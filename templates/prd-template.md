...
> **Falsifiability requirement:** every quantitative AC (coverage %, latency budget, RPS, token delta, line count, etc.) MUST cite a concrete verification command and its exit-code contract inline — for example: `"${DATARIM_RUNTIME:?}/dev-tools/measure-foo.sh" --check` returns exit 0. No «presumed met» verdicts at QA / Compliance. If the measurement tool does not yet exist, declare its creation as part of the plan; do not approve the AC until it is falsifiable. See `skills/evolution/SKILL.md` § Pattern: Split-Architecture Metrics for the absorption-task variant.
...
> **Runtime-script citation root requirement.** Any citation of a Datarim runtime script or skill file anywhere in the PRD body (Requirements, Success Criteria, AC/V-AC verification lines, Risks) MUST carry an explicit root — `${DATARIM_RUNTIME:?}/...` or the framework repo's full canonical path — never a bare-relative form such as `plugins/<name>/scripts/<script>.sh`. A bare-relative path resolves against whichever directory the reading agent's cwd happens to be, which is frequently the consumer workspace root rather than the runtime — so the same path string points at different (or no) files depending on who reads the PRD. This is a citation-form defect distinct from the path-existence checks above: a rooted path can still be validated by `test -f`, but an unrooted one is ambiguous regardless of whether the target exists.
