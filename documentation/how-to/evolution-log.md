...
- **Design (per PRD-TUNE-0490 Phase 2; SRE lens).** Pane liveness alone is insufficient — join it with a synced heartbeat status file that is authoritative for DONE/AWAITING. Three framework-only pieces (no live-host mutation, ship as code):
  - `dev-tools/lib/heartbeat-status.sh` — the single writer/reader of `datarim/runtime/<TASK-ID>.status` (JSON: state ∈ {init,in_progress,awaiting_operator,done}, stage, updated_at wall-clock epoch, pid, optional question/options). Atomic write (tmp+mv), jq-optional read, strict `^[A-Z]{2,10}-[0-9]{4}$` task-id validation (closes a runtime-dir-escape vector — the earlier bash-glob check wrongly accepted `lowercase-0001`), lossless backslash/quote escaping.
  - `dev-tools/classify-pane.sh` — read-only 2-key join (pane tail × status freshness) → RUNNING / AWAITING / STALLED / DONE / DEAD-ORPHAN / HOLD. Safety invariants encoded: awaiting_operator ALWAYS wins (never reaped); a live child ALWAYS blocks DEAD-ORPHAN (→ STALLED, escalate); DEAD-ORPHAN only after ≥2 consecutive stale probes (no single-probe reap). Fully parameterised (`--pane-file`, `--has-child`, `--stale-count`, `--now`) so it is deterministic and testable off fixtures — no tmux/ssh needed to test.
  - `dev-tools/dispatch-digest.sh` — aggregates all synced status files into ONE report generated from status writes (never by prompting the operator), pins actionable states (AWAITING, then in-progress) first, flags `SYNC STALE` when the freshest write is older than the 30-min cadence so a stale-synced task is never falsely reported DONE. Text + `--json` modes for OpsBot/Telegram relay.
- **Tests.** `tests/heartbeat-status.bats` (8/8 — roundtrip, atomic no-turd, task-id escape rejection incl. `../`, escape safety, awaiting question/options) + `tests/classify-pane-digest.bats` (14/14 — V-AC-4 no-false-DONE, the three safety invariants, digest aggregation + SYNC STALE degrade). shellcheck -S warning clean on all three scripts; stack-agnostic gate PASS; gitleaks no-leaks; anti-pattern grep zero. `.gitignore` covers `datarim/runtime/`.
- **Deferred to Phase 2-live / Phase 3 (operator-gated, live-host).** The `answer <TASK-ID> <qid> <index>` and `kill-session dr-<space>-<ID>` whitelisted dispatch verbs, the OpsBot→Telegram relay wiring, and the reaper systemd timer are live-host mutations and stay operator-gated at execution time per PRD §5. The framework substrate they need (the status-file contract + classifier + digest) lands here. Provenance: PRD-TUNE-0490 Phase 2 V-AC-4/5/6.

## 2026-07-14 — TUNE-0490 Phase 1 — Delegated autonomous orchestration via per-task marker (v2.52.0)
...
- **P-1 (skill-update, Class A APPLIED):** `skills/compliance/SKILL.md` Infrastructure checklist gains § 9 «Multi-Root Success-Criterion Verification»: when a wish / AC names ≥2 filesystem roots that must all satisfy the same property, the verification MUST grep every named root independently — a repo-local helper proves only its own repo. Source: SPACE-0013 (Arcanada ecosystem) — a «no working-branch-auto references anywhere in local-env AND spaces/aether» criterion was verified only by the in-repo `check-no-auto-refs.sh` living in `local-env`; the sibling registry tree `spaces/aether/{inventory,runbook}.md` still carried working `git clone -b auto` recipes and reached `/dr-qa` Layer 3b as a BLOCKER. Gates: stack-agnostic PASS (--diff-only), body-english PASS (119 files), bats green (1086 tests, 0 failures). Cross-ecosystem apply: a SPACE task surfaced a framework-runtime improvement.
