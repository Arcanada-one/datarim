...
For L2+ work, successful `/dr-prd`, `/dr-plan`, and `/dr-do` stages verify their saved output before suggesting the next command. L2 uses one read-only peer reviewer. L3/L4 normally use three independent read-only roles; when a valid parent-owned budget signal shows pressure, Datarim follows `full -> deep_only -> floor_only`, dropping the adversarial bundle before the deep reviewer. The deterministic floor always runs. The immutable audit reports `verification_coverage` separately from PASS/CONDITIONAL/BLOCKED, so `floor_only` never implies that model review occurred. If evidence is invalid or a selected reviewer cannot complete, Datarim reports incomplete verification and keeps the route at the current stage instead of silently degrading or presenting a false PASS. L1 skips this work entirely.

**Guarded automatic fixes.** Before reviewer dispatch, the deterministic floor may identify a registered formatting, trailing-whitespace lint, or exact typo finding. Datarim applies it only when the complete immutable history for that exact detector and fixer is strictly below 30% false positives. It applies one finding at a time and reruns the stage validators and floor before considering another or evaluating budget pressure. Missing or uncertain history is advisory only. Every attempted result remains visible in the immutable verification audit beside the unchanged original finding; the normalized degradation decision is recorded there without the raw budget ledger.
