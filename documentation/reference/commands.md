...
| `/dr-prd` | Requirements | architect | Generate and validate a PRD, then apply automatic post-step self-verification before the parent CTA/snapshot. |
| `/dr-plan` | Planning | planner | Create and validate the implementation plan, then apply automatic post-step self-verification before the parent CTA/snapshot. |
| `/dr-design` | Design | architect | Architecture exploration with consilium (L3-4). Emits CTA. |
| `/dr-do` | Execution | developer | Execute with TDD/evidence, then apply automatic post-step self-verification before QA routing. |
...
| `/dr-verify` | Verification | -- | Standalone manual full tri-layer self-verification. The automatic PRD/plan/do path instead uses the shared skill's one-pass `post_step` profile: L1 off, L2 one peer reviewer, L3/L4 three independent parallel roles; incomplete execution is non-advancing. |
