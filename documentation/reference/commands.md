...
| `/dr-prd` | Requirements | architect | Generate and validate a PRD, then apply automatic post-step self-verification, including guarded deterministic auto-fix and the L3/L4 `full -> deep_only -> floor_only` policy when eligible, before the parent CTA/snapshot. |
| `/dr-plan` | Planning | planner | Create and validate the implementation plan, then apply automatic post-step self-verification, including guarded deterministic auto-fix and the L3/L4 `full -> deep_only -> floor_only` policy when eligible, before the parent CTA/snapshot. |
| `/dr-design` | Design | architect | Architecture exploration with consilium (L3-4). Emits CTA. |
| `/dr-do` | Execution | developer | Execute with TDD/evidence, then apply automatic post-step self-verification, including guarded deterministic auto-fix and the L3/L4 `full -> deep_only -> floor_only` policy when eligible, before QA routing. |
...
| `/dr-verify` | Verification | -- | Standalone manual full tri-layer self-verification; manual `/dr-verify` remains findings-only and does not auto-degrade. The automatic PRD/plan/do path uses the shared bounded `post_step` profile: L1 off, L2 one peer reviewer, L3/L4 cost-adaptive coverage after eligible fixes; the deterministic floor is mandatory, `verification_coverage` is audit-visible, and incomplete execution is non-advancing. |
