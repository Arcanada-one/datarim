...
> **Not a core command: `/dr-continue-checkpoint`.** A worker entry for projects
> that run their own external controller, one that pauses a stage on a question
> and resumes it later in a fresh worker, ships as the opt-in
> `plugins/controller-continuation/` plugin (Linux only; other platforms are
> refused with `continuation_unsupported_platform`). It is not installed as a
> slash command and is not counted above. See
> `plugins/controller-continuation/README.md`.
