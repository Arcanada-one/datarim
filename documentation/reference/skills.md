...
| self-verification | Reference | inherit | Canonical manual and automatic verification profiles. Manual `/dr-verify` retains findings-only full tri-layer review. PRD/plan/do use bounded `post_step` dispatch: L1 off, L2 one peer reviewer, and automatic L3/L4 `full -> deep_only -> floor_only` after eligible deterministic fixes. Fix eligibility requires the fixed immutable history namespace to prove exact tuple history strictly below 30% false positives. The deterministic floor is mandatory, `verification_coverage` is audit-visible, and raw budget evidence is never persisted. | `/dr-verify`, `/dr-prd`, `/dr-plan`, `/dr-do` |
