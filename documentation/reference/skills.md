...
| seam-vs-integration-boundary | Reference | inherit | Plan-time scope-boundary pattern: when a one-liner bundles a seam/contract concern (define an interface / dispatcher in module X) with an integration/call-site concern (wire it into a caller in module Y), split into foundation + integration follow-up or scope the ACs so the integration is explicitly optional/deferred; advisory detector `dev-tools/check-seam-integration-boundary.sh` | `/dr-plan` Phase 4 Component Breakdown (seam-vs-integration boundary check) |
...
**Distribution:** 23 reference (inherit), 3 opus, 13 sonnet, 4 haiku.
