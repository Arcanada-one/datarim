# Handoff: the two DEV-BOX test sessions
...
Both sessions live in `tmux` on `client` (ssh alias for DEV-BOX), both working in
`/home/client/code/client/local-stack`, both reporting their task complete.
...
```bash
ssh client 'tmux kill-session -t jev-autopilot'
```
