...
| **writer** | Technical Writer | /reflect, /archive, /prd |
...
**Specialized skills:**
- `consilium.md` — Multi-agent panel discussions (loaded by: /design for L3-4)
- `discovery.md` — Requirements discovery interview (loaded by: /prd)
- `evolution.md` — Framework self-update rules (loaded by: /reflect)
...
Each project maintains a `datarim/` directory at the project root (created by `/init`).
...
3. If not found anywhere: **STOP** — only `/init` may create `datarim/`
...
|---------|-------|-------------|
| `/init` | Initialize | Create task, assess complexity, set up `datarim/` |
| `/prd` | Requirements | Generate PRD with discovery interview |
| `/plan` | Planning | Detailed implementation plan with strategist gate |
| `/design` | Design | Architecture exploration with consilium |
| `/do` | Implementation | TDD development, one method at a time |
| `/qa` | Quality | Multi-layer verification (PRD, design, plan, code) |
| `/compliance` | Hardening | 7-step post-QA hardening |
| `/reflect` | Reflection | Lessons learned + framework evolution proposals |
| `/archive` | Archive | Complete task, update backlog, reset context |
| `/status` | Utility | Check current task and backlog status |
...
Datarim improves itself through the `/reflect` stage:
