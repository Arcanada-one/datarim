...
| `/dr-plugin` | Extension | Manage ordinary opt-in plugins plus trusted metadata toggles: default-on `tdd-enforcement` and default-off `ltm-graph-memory` (list/enable/disable/sync/doctor). Manifest-driven runtime symlinks, workspace policy state, snapshot/rollback, dependency-graph + skill-registry health checks |
