...
- `seam-vs-integration-boundary.md` — plan-time scope-boundary pattern: when an L3+ backlog one-liner bundles a seam/contract concern (define an interface/dispatcher in module X) with an integration/call-site concern (wire it into a caller in module Y), flag the operator to split into foundation + integration follow-up or scope the ACs so the integration is explicitly optional/deferred; advisory detector `dev-tools/check-seam-integration-boundary.sh` (loaded by: /dr-prd, /dr-plan Step 4 scope-boundary probe). Source: reflection-ARAS-0003 evolution-proposal #3.
...
- `adversarial-review.md` — General adversarial-mindset review: attack an artifact (plan/PRD/code/design) to break it, not bless it; "no findings" is an alarm, not an all-clear (loaded by: /dr-plan Transition Checkpoint, /dr-qa Layer 4c-bis).
- `edge-case-hunter.md` — Boundary/degenerate/failure-input enumeration: cardinality, emptiness, ordering, concurrency, partial failure, malformed input; ranks silent-failure edges first (loaded by: /dr-plan Validation Checklist, /dr-qa; sibling of adversarial-review).
- `structure-review.md` — Structural/editorial review: completeness, internal consistency, traceability, drifted counts, un-runnable step order — does the artifact hold together as a document (loaded by: /dr-plan, /dr-qa Layer 3; sibling of adversarial-review).

Skill files: `$HOME/.claude/skills/{name}/SKILL.md` (68 skills, 17 with supporting fragment directories — a "supporting fragment directory" is a skill folder that ships at least one sibling `.md` beside its `SKILL.md`)
