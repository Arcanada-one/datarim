...
Skill files: `$HOME/.claude/skills/{name}/SKILL.md` (63 skills, 15 with supporting fragment directories — a "supporting fragment directory" is a skill folder that ships at least one sibling `.md` beside its `SKILL.md`)
...
| `/dr-plugin` | Extension | Manage ordinary opt-in plugins plus trusted metadata toggles: default-on `tdd-enforcement` and `coworker-delegation`, and default-off `ltm-graph-memory` (list/enable/disable/sync/doctor). Manifest-driven runtime symlinks, workspace policy state, snapshot/rollback, dependency-graph + skill-registry health checks |
| `/dr-orchestrate run` | Core+Plugin | Self-driving Datarim pipeline runner. **Command and autonomy policy are core** (no plugin needed — `dev-tools/rules/fb-rules.yaml` + `dev-tools/fb-policy-loader.sh`). Phase 1 is rule-based; Phase 2 adds multi-backend inference and audit schema v2; Phase 3 adds actor/session-confirmed learned rules with 24-hour re-validation and an immutable seven-day TTL, plus default-off Claude Code/Codex context self-clearing that checkpoints task pointers before fixed `/compact` or `/clear` and resumes through snapshot-first `/dr-next`. Learned actions must already be trusted, pass per-space policy, and remain subject to the immutable hard-gated floor. **The tmux/bot transport runner is the opt-in plugin** — `dr-plugin enable <abs-path>/plugins/dr-orchestrate`. Security floor: whitelist + 0x1b escape block + cooldowns + fail-closed pane blocking. JSONL audit stores hashes instead of raw pane text or callback capabilities. |
...
Successful `/dr-prd`, `/dr-plan`, and `/dr-do` stages apply the canonical self-verification skill's internal `post_step` profile after saving and validating their artifact, but before selecting the parent CTA and snapshot. L1 performs no verification work. L2 runs the deterministic floor plus exactly one `peer-reviewer`. L3/L4 normally run the floor plus `reviewer`, `tester`, and `security`; valid parent-owned token/cost pressure may reduce only through `full -> deep_only -> floor_only`, dropping the adversarial bundle before the deep reviewer. The deterministic floor is never skipped, and `verification_coverage` records the completed tier separately from the canonical verdict. For L2+, the parent may auto-apply only registered deterministic formatting, trailing-whitespace lint, or exact-typo fixes when complete immutable class history is strictly below 30% false positives. It applies one finding, reruns validators and the floor, and only then evaluates pressure. Every decision is recorded in the immutable audit with normalized signal fields; the raw budget ledger is not persisted. Invalid evidence or reviewer/provider failure records `execution_status: incomplete` rather than silently degrading. The manual `/dr-verify` remains findings-only and never enters the mutation or adaptive automatic path.
...
**Status:** automatic `post_step` and manual tri-layer profiles are canonical. The automatic L2+ profile has the guarded deterministic auto-fix boundary described above; automatic L3/L4 also has the audited cost-adaptive lattice, while manual `/dr-verify` remains findings-only. Cross-link: skill `skills/self-verification/SKILL.md` · floor script `dev-tools/dr-verify-floor.sh` · auto-fix runner `dev-tools/self-verify-auto-fix.sh` · degradation evaluator `dev-tools/self-verify-degradation-policy.sh` · template `templates/archive-template.md`.
...
**Manifest:** `datarim/enabled-plugins.md` — single source of truth (one entry per active ordinary plugin plus trusted policy state). The metadata-only `tdd-enforcement` plugin is enabled by default; its tombstone makes strict test-first sequencing optional without weakening tests or downstream gates. The metadata-only `coworker-delegation` plugin is also enabled by default; its tombstone permits native agent I/O while preserving unrelated hook safety controls. The metadata-only `ltm-graph-memory` plugin is disabled by default; one exact built-in active record permits a separately configured LTM adapter, while missing or invalid state forbids graph-memory I/O. These policy toggles create no runtime symlinks.
...
- Validation gate rejects: invalid plugin id (must be kebab-case, ≤32 chars), embedded credentials in URLs, control characters in sources, CRLF in `plugin.yaml` (security), path traversal (`..`), schema_version drift (only `1` accepted).
- Critical-core overrides (`evolution`, `datarim-system`, `pre-archive-check`) emit a warning to stderr and proceed — operator decides.

**Doctor checks (10):** manifest-syntax, inventory-consistency, broken-symlinks, orphan-files, override-integrity, dependency-graph (DFS cycle/dangling), git-state (uncommitted manifest), snapshot-cleanup (>30d), skill-registry (frontmatter `name:` ↔ basename — closes Skill-tool resolution gap), trusted-policy-state (TDD and coworker tombstones plus the LTM metadata record).
