...
|---------|-------|-------------|
| `/dr-init` 🔒 | Initialize | **operator-only** (agents cannot invoke). Start a new task, pick from backlog, or **scaffold a new project**. Assigns complexity (L1-L4) for tasks; for projects use `/dr-init create project "Name"`. |
...
| `/dr-archive` 🔒 | Archive | **operator-only** (agents cannot invoke). Archive the completed task. Performs reflection (Step 0.5) + evolution proposals, then stores context and updates backlog. |
