...
Manage opt-in plugins for the Datarim framework. The current Datarim shipping set (skills, agents, commands, templates) is migrated into a single protected `datarim-core` entry on first run. Third-party plugins are installed as symlinks from `datarim/plugin-storage/<id>/` into the project runtime, `.datarim-runtime/local/{skills,agents,commands,templates}/<plugin-id>/`; nothing is written under your home directory.
