...
2.  **RESOLVE PATH**: This is the ONLY command that may create `datarim/`. Resolve the correct location:
    ...
    - If creating for the first time:
      ...
      c. Keep `datarim/` out of git **without editing `.gitignore`** — the project's `.gitignore` is shared with people who may not run Datarim. Run `git check-ignore -q datarim/`; if it already reports ignored (the installer normally does this), do nothing. Otherwise append `/datarim/` to the clone-local exclude file at `$(git rev-parse --git-path info/exclude)`, which is never committed.
...
3.6. **BACKLOG-ENTRY STALENESS PROBE** (advisory, non-blocking; backlog-selection flow only, runs immediately after Step 3.5, before any `tasks.md`/`activeContext.md` write):
    - **Trigger — ALL three conditions on the picked entry**: (a) priority `P1` or `P2`; (b) age above ~14 days — read an inline date annotation in the row when present, otherwise take the timestamp of the commit that first introduced the entry's ID into `backlog.md` (e.g. `git log --format=%ct -S "{TASK-ID}" -- datarim/backlog.md | tail -1`; skip silently when `backlog.md` is not git-tracked); (c) the description names concrete files, paths, scripts, or mechanisms (a `path/to/file` reference, a named script/gate/step/section, a quoted line range).
    - Skip silently when any condition fails — fresh entries, P3+ entries, and purely conceptual descriptions are unaffected. Ops-fire-shaped symptoms (production down, restart loops, live-fire wording) are already covered by the Step 2.5e symptom-freshness re-probe — do NOT duplicate that probe here; this step covers the general aged-entry class, 2.5e covers the live-symptom class.
    - **When triggered, BEFORE accepting the brief as fact**: live-probe every named path/mechanism — existence first (`test -e` / `git cat-file -e`), then a quick grep for the named mechanism (does the referenced step/gate/section still exist in the named file? has the described gap already been closed by an intervening task?). A high-priority entry that sat for weeks while parallel work shipped is the highest-risk class for a brief that no longer matches reality.
    - Print a **staleness advisory** summarising per-claim verdicts, e.g.: `STALENESS ADVISORY: entry is <N> days old; probed <M> named references — <K> confirmed current, <J> drifted (<path>: mechanism already shipped / renamed / removed)`.
    - **All probed claims already satisfied** (the described work appears to have shipped in the interim): recommend closing the entry as superseded/stale instead of initialising it — same disposition as Step 2.5e; the operator decides.
    - **Some claims drifted but work remains**: continue to Step 4, but record the per-claim drift verdicts in the task's Overview section so `/dr-prd`/`/dr-plan` inherit the corrected ground truth instead of the stale brief.
    - **Probe impossible** (named paths unreachable from this session): continue unchanged — this step never blocks, it only redirects or annotates.
    - Cost: seconds of read-only shell probes. Saving: avoids initialising and planning a task whose substance already shipped between backlog capture and `/dr-init` time. The deeper `/dr-plan` Phase 4 "Backlog-brief staleness probe" (MANDATORY for L2+ file:mechanism claims) still runs later; this advisory catches the fully-superseded case before any artefact is created.
...
4.  **ACTION**:
    ...
    - **Determine Task ID** (if NOT from backlog): select prefix per Unified Task Numbering (`${DATARIM_RUNTIME:?}/skills/datarim-system/SKILL.md`) — project prefix first, then area prefix, `TASK` as fallback. Then assign the next free ID by **running the canonical helper** (do NOT compute `max+1` mentally):
      `"${DATARIM_RUNTIME:?}/dev-tools/next-free-id.sh" {PREFIX} "$DATARIM_ROOT"`
    ...
    - **Atomic ID claim (MANDATORY — run this immediately after the helper prints the ID, before ANY artefact write):** `next-free-id.sh` is best-effort: two sessions can compute the same `max+1` candidate and both pass every read-only probe before either writes `tasks.md`. Close that TOCTOU window by claiming the ID atomically:
      `bash "${DATARIM_RUNTIME:?}/dev-tools/dr-init-id-lock.sh" acquire {TASK-ID} "$DATARIM_ROOT"`
      ...
      `bash "${DATARIM_RUNTIME:?}/dev-tools/dr-init-id-lock.sh" release {TASK-ID} "$DATARIM_ROOT"`
    ...
    - **If new project/service**: Load `${DATARIM_RUNTIME:?}/skills/tech-stack/SKILL.md` and identify the starting-point stack from the Default Recommendation column. For non-routine cases (cross-domain, stack-migration, operator-request), generate a Stack Proposal with 2-3 candidates with trade-offs per the Trigger Classifier; the operator chooses and the choice is recorded as a decision note in the plan.
    - Create/Update `datarim/tasks.md` with new task.
    - **Append** new task to `## Active Tasks` in `datarim/activeContext.md`. Do NOT remove existing active tasks. If `activeContext.md` uses legacy format (`**Current Task:**` single line), convert to `## Active Tasks` list first. See `${DATARIM_RUNTIME:?}/skills/datarim-system/SKILL.md` § activeContext.md Write Rules.
    - **Stage Header (header after Step 4)**: From this point onward in the response (after the TASK-ID has been determined), emit `**{TASK-ID} · {title}**` as the first line of the post-Step-4 message block per `${DATARIM_RUNTIME:?}/skills/cta-format/SKILL.md` § Stage Header. Do NOT emit the header during Steps 0-3 (TASK-ID is not yet known). Single occurrence per command invocation.
4.6. **WRITE INIT-TASK FILE** (mandatory, F1 contract — see `${DATARIM_RUNTIME:?}/skills/init-task-persistence/SKILL.md`):
    ...
    - Probe: `bash "${DATARIM_RUNTIME:?}/dev-tools/check-init-task-presence.sh" --task {TASK-ID} --root "$DATARIM_ROOT"` (where `$DATARIM_ROOT` is the parent of `datarim/` and `$DATARIM_RUNTIME` is the installed runtime root; falls back to `~/.claude` for default-symlinked installs that include `dev-tools/` in `INSTALL_SCOPES`). Exit 0 = OK; non-zero = print warning and continue (operator may amend manually).
    ...
    - **License auto-sync**: when the Q&A round-trip (`skills/init-task-persistence/SKILL.md` § Q&A round-trip contract) records an operator license decision for this project, update the project's root `README.md` — replace the "License" section's "TBD" placeholder with the **canonical render** of the chosen license (name + SPDX identifier where applicable).
      - **Match the render against the project manifest**: when the project ships a machine-readable manifest that declares a license field, read the SPDX expression from that manifest and render the README section from it — a single license renders as one sentence naming the license and its SPDX identifier; a dual/multi license (an SPDX `OR` expression) renders as one line per option plus the standard at-your-option sentence. The manifest is the source of truth: after the sync, the README "License" section and the manifest license field MUST agree. If the operator decision contradicts the manifest, update the manifest in the same edit (or surface the conflict to the operator) — never ship a divergent README/manifest pair.
    <!-- gate:example-only -->
      - Manifest examples (illustrative — use the project's actual manifest): a Rust workspace declares the field at `[workspace.package].license` in `Cargo.toml`; a Node package declares it as the `"license"` key in `package.json`.
    <!-- /gate:example-only -->
      - Skip silently when `README.md` has no "License" section, or the section already names a concrete license (idempotent — never overwrite an operator-set value).
...
4.65. **CAPTURE FRAMEWORK VERSION BASELINE** (mandatory when the resolved
    ...
    - Immediately after the canonical init-task passes its structural probe,
      ...
      ```bash
      "${DATARIM_RUNTIME:?}/dev-tools/capture-framework-version-baseline.sh" \
...
4.7. **WRITE EXPECTATIONS SKELETON** (mandatory for all complexity levels L1-L4 — see `${DATARIM_RUNTIME:?}/skills/expectations-checklist/SKILL.md` § When the file is created):
...
After completing initialization, the planner agent MUST emit a CTA block ([definition](../skills/cta-format/SKILL.md)) per `${DATARIM_RUNTIME:?}/skills/cta-format/SKILL.md`.
