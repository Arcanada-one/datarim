...
4.  **ACTION**:
    ...
    - **Determine Task ID** (if NOT from backlog): select prefix per Unified Task Numbering (`$HOME/.claude/skills/datarim-system/SKILL.md`) — project prefix first, then area prefix, `TASK` as fallback. Then assign the next free ID by **running the canonical helper** (do NOT compute `max+1` mentally):
      ...
      where `$DATARIM_ROOT` is the workspace root (the parent of `datarim/`). The helper applies the canonical formula `max(claimed across documentation/archive ∪ datarim/tasks.md ∪ datarim/backlog.md) + 1` and additionally counts in-flight init-lock markers as a 4th claim surface (`datarim/.locks/{ID}.init-lock`), so a concurrent session's claimed-but-unwritten ID is visible before it reaches `tasks.md`. It auto-bumps on a parallel-session race, printing the chosen `PREFIX-NNNN` to stdout. **Documented fallback** (helper unavailable in this runtime): compute the same formula by hand.
    - **ID-collision probe (MANDATORY)**: **Do not emit or announce the chosen task ID — in reply text or in any artefact — until this collision probe completes.** The helper's grep IS the collision probe for the agent's own new ID. Additionally probe for foreign entries: `grep -lE "^- {TASK-ID} ·" datarim/backlog.md datarim/tasks.md 2>/dev/null` AND `ls documentation/archive/*/archive-{TASK-ID}.md 2>/dev/null`.
      ...
      - **FOREIGN entry (someone else's queued work):** If ANY match appears for a FOREIGN entry — STOP and present a 3-way prompt to the operator: **(a) reassign the prior backlog/queued entry to the next free ID** (update both occurrence + any cross-references; recommended when the prior entry is `pending` and lower-priority); **(b) cancel the prior entry** (delete from backlog with a one-line rationale); **(c) operator picks a different ID for the new task**. Do not proceed with `{TASK-ID}` until the collision is closed. Rationale: backlog ID-uniqueness ≠ tasks.md ID-uniqueness — gates downstream `/dr-archive` Step 3 against silent overwrite of an unrelated queued unit of work.
    - **CLAIM THE ID ATOMICALLY (MANDATORY, race-window close):** immediately after the probe passes and **before emitting/announcing the ID or writing any artefact**, acquire an atomic claim marker so a parallel session cannot write the same ID in the window before `tasks.md` exists:
      `bash "${DATARIM_RUNTIME:-$HOME/.claude}/dev-tools/dr-init-id-lock.sh" acquire {TASK-ID} "$DATARIM_ROOT"`
      - **exit 0** → the marker (`datarim/.locks/{TASK-ID}.init-lock`) is now held by this session; proceed. Hold it until Step 4.8.
      - **exit 1 (COLLISION)** → a live session claimed this ID between the probe and now. Re-run `next-free-id.sh` (which counts the marker as surface 4), take the new ID, and retry `acquire`. Repeat until exit 0.
      - **Documented fallback** (helper unavailable in this runtime): skip the marker; the `next-free-id.sh` re-probe remains the best-effort defence. Note the degraded guarantee in the reply.
...
4.8. **RELEASE ID-LOCK** (mandatory when Step 4 acquired the marker): once `tasks.md`, `activeContext.md`, the init-task file, and the expectations skeleton are all written, the ID is now claimed on the durable surfaces and the marker is no longer needed. Release it:
    `bash "${DATARIM_RUNTIME:-$HOME/.claude}/dev-tools/dr-init-id-lock.sh" release {TASK-ID} "$DATARIM_ROOT"`
    Release is idempotent (exit 0 whether or not the marker existed). A marker left behind on a crash between acquire and release self-expires after `DR_INIT_LOCK_TTL` (default 900s) and is then reclaimable by the next `acquire`, so a wedged ID is bounded, not permanent.
