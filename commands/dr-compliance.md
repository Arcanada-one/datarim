...
6.2. **RE-ASSERT EVIDENCE ATTRIBUTION GATES** (conditional, fail-hard):
    - **Shell-harness Gate 9:** when a status-transforming shell harness supplies evidence, re-run the positive and negative child controls required by `skills/testing/live-smoke-gates.md` § Gate 9. For expected-red/mutation evidence, require the exact named assertion and prove an unrelated setup failure becomes `HARNESS_INVALID`. Missing proof makes the verdict **NON-COMPLIANT**.
    - When changed code maps a unique-violation family to a domain result, re-assert `skills/testing/SKILL.md` § Unique-Violation Domain Normalization: exact named constraint or exact normalized column signature; runtime-shaped unit and mapping-boundary evidence; intended positive and unrelated unique-violation negative; real-database evidence when metadata is driver/schema dependent. Unknown explicit names must fail loud without column fallback. Missing or mock-only required proof makes the verdict **NON-COMPLIANT**, never notes.
6.3. **HISTORY-AGNOSTIC GATE — task-ID provenance scan (MANDATORY when the task touched any shipped runtime markdown — `skills/`, `agents/`, `commands/`, or `templates/` of a Datarim-style framework repo; skip silently otherwise)**:
...
The CTA block MUST follow canonical FAIL-Routing format when NON-COMPLIANT — the header uses the FAIL-Routing CTA header phrasing defined in `cta-format.md` § FAIL-Routing, reporting the NON-COMPLIANT verdict for this task and the earliest failed layer (`Layer N (Layer name)`). Variant B menu when >1 active tasks.
