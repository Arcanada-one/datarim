...
Активирует существующие mandates FB-1..8 как default-on. Делегирует подавление Q&A в `skills/autonomous-mode.md`. Моды: Continue (resume task) / Bootstrap (full pipeline).

**Role**: Adaptive (planner/architect/developer per dispatched stage)
**Activates skill**: `${DATARIM_RUNTIME:-$HOME/.claude}/skills/autonomous-mode.md`

## Instructions

**Stage Header (mandatory)**: Emit `**{TASK-ID} · {title}**` as first line per `skills/cta-format.md` § Stage Header.

0. **RESOLVE PATH**: Walk-up к `datarim/` per `skills/datarim-system.md` § Path Resolution Rule. If not found → STOP, tell operator run `/dr-init`.
...
2. **ACTIVATE AUTO-MODE**:
   ...
   - Write marker file `datarim/.auto-mode-active` (YAML per `skills/autonomous-mode.md` § When this skill is active):
     ```yaml
     task_id: "{TASK-ID}"
...
6. **L1 INLINE GAP CAPTURE**: gaps discovered mid-pipeline → classify per L1 Inline Rule (decision tree в skills/autonomous-mode.md). Inline-resolved gaps append в `datarim/tasks/{TASK-ID}-auto-inline-log.md` с timestamp + files-touched + LoC delta + classification rationale.
...
8. **TERMINAL CLEANUP**: at success / hard-stop:
   ...
   - Emit CTA block per `skills/cta-format.md` § CTA format.
   - Emit Stage Snapshot per `skills/cta-format.md` § Snapshot Emission (`stage: auto`, `command: /dr-auto`).
...
Verbatim из `documentation/mandates/autonomous-agents.md:30-32` — full list в `skills/autonomous-mode.md` § Hard-gated Action Boundary. Краткий summary:
...
After terminal stage, the agent MUST emit CTA block per `skills/cta-format.md`.
...
After CTA block, emit snapshot per `skills/cta-format.md` § Snapshot Emission:
- `stage`: `auto`
- `command`: `/dr-auto`
- `captured-by`: `agent`
