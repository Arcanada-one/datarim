...
0.  **RESOLVE PATH**: Before any read/write to `datarim/`, find the correct path by walking up directories from cwd. If `datarim/` is not found anywhere, STOP and tell user to run `/dr-init`. Do NOT create it — only `/dr-init` may create `datarim/`. See `${DATARIM_RUNTIME:?}/skills/datarim-system/SKILL.md` § Path Resolution Rule.
...
0.8. **SKILL**: Also load `${DATARIM_RUNTIME:?}/skills/immutability/SKILL.md` — read the `/dr-plan Rules` fragment for plan artefact immutability, V-AC parity (V-AC = verifiable acceptance criterion — an acceptance criterion paired with a concrete runnable check, [definition](../skills/v-ac-feasibility/SKILL.md)), and Return-to-Source routing.

1.  **TASK RESOLUTION**: Apply Task Resolution Rule from `${DATARIM_RUNTIME:?}/skills/datarim-system/SKILL.md` § Task Resolution Rule. Use the resolved task ID for all subsequent steps.

1.5. **READ INIT-TASK** (mandatory per `${DATARIM_RUNTIME:?}/skills/init-task-persistence/SKILL.md`): Open `datarim/tasks/{TASK-ID}-init-task.md` if present. Read the full `## Operator brief (verbatim)` section AND every `## Append-log` entry. Any divergence between the operator's stated intent and the planned scope MUST be recorded in the plan's § Notes / § Risks. Missing init-task is non-blocking — flag as advisory and continue.
...
3.  **Strategist Gate** (mandatory for L3-4 and for redundancy-only or ambiguous scope at every complexity):
    ...
    3.  **Invoke exactly once when required.** Load
        `${DATARIM_RUNTIME:?}/agents/strategist.md`.
    ...
    4.  **Normalize and validate the closed record.** Write exactly the 18
        ...
        ```bash
        bash "${DATARIM_RUNTIME:?}/dev-tools/check-strategist-gate-record.sh" \
          --root <workspace-root> \
...
4.  **Detailed Design (Phase 4)**:
    ...
    -   **Backlog-brief staleness probe (MANDATORY for L2+ when the backlog one-liner or init-task brief names a specific `file:mechanism` claim — "X uses Y", "Z still does W" — about live infrastructure).** A brief is a snapshot taken when the item was written to the backlog, not when `/dr-plan` runs; the gap between those two dates can span weeks, and the named mechanism may have already migrated (a `deploy.yml` claimed to use one auth token scheme had already migrated to another by the time its backlog item was planned). Before treating the brief's `file:mechanism` claim as current fact, live-probe the named file for the named mechanism (`grep`/`Read` the cited file, or the smallest command that exercises it) and record the result inline in the plan's Overview / Decisions section: confirmed-current (`file:line still shows <mechanism>`) or stale (`file:line now shows <mechanism-B>` → re-scope or cancel before component breakdown). Cost: a single grep + skim. Saving: a full `/dr-do` cycle spent implementing a fix for a problem that already resolved itself between backlog entry and plan time.
    ...
    -   **Deliverable-to-V-AC binding (MANDATORY)**: every artefact listed in Component Breakdown as new or modified (a Deliverable Artifact) MUST have a corresponding row in the plan's Validation Checklist (Phase 5 below) that verifies it, at minimum a presence-check (`test -f <path>`). A deliverable with no V-AC row is a planning defect — `/dr-qa` Layer 1 (`dr-spec-lint.sh` rule `vac-deliverable-coverage`) will flag every Component Breakdown artifact with no matching V-AC row as a warning. The gate is advisory by default; hard mode (`DATARIM_SPEC_GRAPH_MODE=hard`) blocks transition to `/dr-do`. Fix: either add the V-AC row or drop the artefact from Component Breakdown. Rationale: an unverified deliverable can silently drop out of the implementation without any gate catching it (inventory-drift class).
    -   **Seam-vs-integration boundary check (MANDATORY for L3-L4 tasks; recommended for L2)**: before locking Component Breakdown, re-read the backlog one-liner (and PRD Problem Statement, if present) and check whether it names TWO discrete concerns bundled into one task — e.g. defining a seam/contract in crate (or module/package) X AND wiring the integration call site that consumes it in crate (or module/package) Y. Signal words: "contract in X + call it from Y", "define interface AND wire consumer", "X exposes / Y calls X". If both concerns are present, the plan MUST NOT smear them across shared Implementation Steps. Instead:
        -   Split into explicit separate phases or deliverables — e.g. "Phase 1: seam contract in crate X" and "Phase 2: integration in crate Y" — each with its own Validation Checklist rows and V-AC markers, OR
        -   Flag the operator with a concrete recommendation: split into two tasks (foundation seam-contract task + integration follow-up task), or keep as one task but mark the integration ACs explicitly optional/deferred in the Validation Checklist.
        -   Record the decision (split-into-phases / split-into-tasks / deferred-AC) inline in the plan's Overview or Decisions section, citing the two concerns and the crates/modules they live in.
        -   Rationale: a plan that treats "define the seam" and "consume the seam" as one undifferentiated Implementation Step hides a hard dependency ordering (the seam must compile and stabilize before the integration call site can be written against it) and blurs which V-AC belongs to which concern — a reviewer cannot tell if a failing V-AC is a contract defect or a call-site defect. A 30-second re-read of the backlog wording against the crate/module boundary catches this before `/dr-do`.
        -   **Run the detector first (MANDATORY on L3-L4, before locking Component Breakdown):** `bash "${DATARIM_RUNTIME:?}/dev-tools/check-seam-integration-boundary.sh" --task {TASK-ID} --root "$DATARIM_ROOT" --report` (or `--line "<one-liner text>"` when the one-liner is not in the backlog). It is a narrow, deterministic, **advisory** scanner — exit 0 whether it flags or not, so it never blocks a plan; `--strict` exits 3 for a caller that wants a hard signal. An `ADVISORY: seam+integration boundary` verdict with its per-class signal report is an input to the judgement above, never a substitute for it: the planner still makes the scope call. Its two signal lists are a **floor, not a ceiling** — a `PASS: single concern` does NOT discharge this check, because a one-liner can mix concerns in wording neither list catches. Contract and signal lists: `${DATARIM_RUNTIME:?}/skills/seam-vs-integration-boundary/SKILL.md`. Skip silently when the helper is absent (older install) and do the check by reading.
    -   **Deploy-gated design cross-reference (MANDATORY when a plan step cannot execute until a deploy that a `/dr-design` decision governs has landed).** When an implementation step depends on a rollout whose go/no-go or shape is decided in a creative doc, annotate that step `[deploy-gated — see creative-{ID}.md § Decision]` so the plan↔creative dependency is explicit and QA can confirm the gate before the step is marked done. The reciprocal marker lives in the creative doc's Decision section (see `/dr-design`).
...
5b. **Append-merge expectations checklist (L2 without PRD, plan-driven additions only)** per `${DATARIM_RUNTIME:?}/skills/expectations-checklist/SKILL.md` § Append-merge contract:
...
6.5.  **Symbol Existence Check (MANDATORY when the plan names a method, function, file, flag, env var, or CLI command as a fix target)**:
    ...
    -   **Standard-library symbol/module coverage (ADVISORY — NOTE, not a hard fail).** The base check above greps the project tree, which by construction cannot resolve a symbol or module that ships in the language's standard library rather than in project source — a plain grep for it will report "not found" even when the reference is correct. When the plan names a standard-library module, function, or type as part of the fix (e.g. an import/require/use statement resolving outside the project's own source tree), do not fail the plan on a project-grep miss alone. Instead: (a) confirm the symbol is plausible for the target language/runtime version in scope (spot-check via the language's own doc lookup, `python3 -c "import X"` / `node -e "require('X')"` / equivalent no-op probe, or the runtime's package index for the stdlib), and (b) record a one-line advisory note inline in the plan (`stdlib check — <symbol> plausible for <language/version>, not project-grepped`) so `/dr-do` and reviewers know the surface was sanity-checked, not skipped. A wrong or renamed stdlib symbol (e.g. a Python 2 module absent in Python 3, a deprecated Node core module) still deserves a plan-time flag — this bullet lowers the check from a MANDATORY grep-and-cite to an ADVISORY sanity note, because static existence proof for stdlib symbols is not reliably available across stacks without running the target runtime.
    -   **Exit-code namespace probe (MANDATORY when a plan step introduces a NEW exit code on a CLI binary).** Before committing to a new exit-code value, catalogue the codes the target already returns: `grep -E "return [0-9]+|sys\.exit\([0-9]+\)" <target-source>` (adapt the pattern to the target language's return/raise idiom — e.g. `std::process::exit(` for Rust, `process.exit(` for Node). Cite the existing namespace inline in the plan and pick an unused value; a collision silently reassigns an already-documented exit code's meaning and breaks callers that branch on it. Cost: ~5 seconds; saving: a retroactive PRD/plan rewrite once the collision surfaces downstream.
    ...
    -   **Bats test-ID collision pre-flight (MANDATORY when the plan adds new `T<n>`-style row IDs — e.g. `T12`, `T13` — to an EXISTING bats file rather than creating a new file).** A plan that hands out the next sequential test ID by eyeballing the file can pick an ID already in use if a prior task landed rows out of order or the file was edited since the plan's context snapshot was taken. Before locking each new row's ID into the Validation Checklist, probe for a collision: `id="T12"; grep -cE "@test \".*\\b${id}\\b" tests/<file>.bats` (substitute the real ID and target file). A non-zero count means the ID is already claimed. On collision, either (a) auto-bump the new row to the next free ID and re-grep to confirm the bumped ID is also clear, or (b) if the plan intentionally supersedes the existing row, document the rename of the existing row (old ID → new ID, with the file:line) in the plan's Implementation Steps so `/dr-do` renames it rather than duplicating the ID. Cost: one grep per new row ID (~2 seconds). Saving: a duplicate `@test` ID silently shadows one of the two tests in bats' human-readable output and CI summaries, so a real regression in the shadowed test can go unnoticed until it resurfaces as an unrelated failure.
    -   **V-AC runtime feasibility probe (MANDATORY when any Validation Checklist row's verification is a runtime command — `docker exec`, `kubectl exec`, `curl`/Playwright against a running service, `systemctl`/`journalctl`, or a live `redis-cli`/`psql`/`mongosh` query).** Semantic AC↔V-AC match (the bullet above) proves the command tests the *right contract*; it does NOT prove the command *can PASS under a correct implementation*. A runtime command can be infeasible by runtime semantics — e.g. `docker exec C printenv X` never observes a value set via `process.env[X]=Y` inside a running Node process, because `printenv` reads the shell env, not the live process env — yet such a V-AC survives a text-only review. Before locking each runtime-command V-AC, prove feasibility per `${DATARIM_RUNTIME:?}/skills/v-ac-feasibility/SKILL.md`: dry-run the command against a real dev/staging/test runtime (or a skeleton) under a deliberately-correct fixture, OR name the exact observation path (log line / health-endpoint field / persisted side-effect) through which the asserted value becomes observable to the command. Annotate the verdict inline (`V-AC-N — feasible (dry-run …)` / `feasible (observation path: …)` / `infeasible as written → re-scoped to …`). A runtime-command V-AC with no feasibility annotation is a planning defect — re-scope to an observable assertion before `/dr-do`. Cost: one dry-run or one observation-path statement. Saving: a full pipeline cycle — an infeasible V-AC otherwise surfaces only at `/dr-do` / `/dr-verify` as a test that cannot pass however correct the code.
    -   **V-AC post-implementation authoring order (MANDATORY when a Validation Checklist row's verification inspects audit-schema fields or the output shape of a command — e.g. `jq '.foo'` against an emitted JSONL/JSON record, or a CLI's stdout/exit-code contract).** A probe that asserts a field or shape drafted purely from the spec, before any live run of the real implementation, can pass a text-only review while checking a field that does not exist in the actual output (F1-class defect: the probe is internally consistent but never observes the real system). This differs from the runtime-feasibility bullet above — the command itself is feasible to *run*, but the field/shape it asserts was never checked against a live emission. The V-AC verification command for these rows MUST be authored or re-verified AFTER at least one live run of the target command/schema producer (a dry-run against a deliberately-correct fixture is sufficient), not drafted blind from the spec and locked before `/dr-do`. Annotate inline (`V-AC-N — verified post-live-run (<command>)`). Cost: one live run per affected row. Saving: an F1-class probe-vs-reality mismatch otherwise surfaces only at `/dr-do` / `/dr-qa` as a spuriously failing gate.
    ...
    -   **History-agnostic runtime-body probe (MANDATORY when Implementation Steps name any file under `skills/`, `agents/`, `commands/`, or `templates/` of a Datarim-style framework runtime as an edit or create target).** The framework runtime is history-agnostic by contract: shipped rule bodies MUST NOT embed task-ID provenance (`Source: <TASK-ID>`, `Per <TASK-ID>`, `<TASK-ID> introduced …`) — such references couple a rule to an ephemeral identifier, distract the reading agent, and risk leaking a historical ID into an end-user-facing AI output. Today this is enforced only downstream (`/dr-do` post-phase-commit pre-flight, `/dr-qa`, `/dr-compliance`, CI `task-id-gate`), so a plan that *prescribes* writing provenance into a shipped body is not caught until a full QA cycle has been spent. Shift the probe left: before locking a runtime-body edit into the plan, apply the deterministic gate contract per `${DATARIM_RUNTIME:?}/skills/evolution/history-agnostic-gate.md` — verify (a) no Implementation Step or example text the plan will ship into a runtime body carries a task-ID provenance token, and (b) every file/script/path the plan cites as an edit or rollback target resolves via the exists + deprecation ladder in `${DATARIM_RUNTIME:?}/skills/plan-path-validator/SKILL.md` (`git cat-file -e`/`test -e` for existence, deprecation-marker grep for staleness) — reuse that skill rather than re-deriving the probe. Annotate the verdict inline (`runtime-body probe — clean, no provenance; N path refs present, 0 deprecated` or `provenance leak in Step N example → rephrase to abstract prose before /dr-do`). A runtime-body edit-target with no history-agnostic annotation is a planning defect — resolve it at plan time, not at `/dr-qa`. Cost: one `scripts/task-id-gate.sh <touched-paths>` dry-run plus the path-validator ladder (~10 seconds). Saving: a full QA-failure-routing cycle — a provenance leak or phantom runtime path otherwise surfaces only at `/dr-qa`/`/dr-compliance` and forces a separate late fix.
<!-- gate:history-allowed -->
    -   **Install-topology survey gate (MANDATORY when the plan fixates a path-resolution canon — env-var, runtime root, install path, template ref, config-file location — that consumer agents will copy literally from runtime markdown).** A canon that works for the default install can silently miss every other install mode. Before locking the canonical form into the plan, survey ALL install topologies the runtime supports and verify the proposed canon resolves correctly in each. For frameworks (e.g. Datarim under `code/datarim/install.sh`), this means: (a) default symlink install (`./install.sh` no flags → runtime at `${DATARIM_RUNTIME:?}/`); (b) per-project install (`./install.sh --project DIR` → runtime at custom location); (c) copy-mode with custom `CLAUDE_DIR` env override; (d) plugin overlays mounting under a runtime root. Cite the survey result inline in the plan (`canon X works for default symlink ✓ / fails for --project DIR ✗ → escalate to env-var fallback form Y`). Heuristic: if the existing canonical pattern for adjacent surfaces uses an env-var fallback (e.g. `${RUNTIME_VAR:-$HOME/.claude}/...`), prefer that form for new path refs too — silent precedent is canonical. A prior default-only template path required a broad migration to `${DATARIM_RUNTIME:?}/templates/X` after another install topology exposed the blind spot. Cost: ~30 seconds (grep install script for mode flags + grep adjacent surfaces for existing env-var patterns). Saving: avoids a full canon-correction cycle.
<!-- /gate:history-allowed -->
...
7.  **Installer / Deploy-Script Content-Type + Directory-Exclusion Audit (MANDATORY when plan touches install.sh, sync-script, or any deploy/copy tool)**:
    ...
    -   **Directory-level exclusions (in addition to the content-type filter above):** grep the script for directory-level exclusion/inclusion rules (`--exclude=`, `.deployignore`, path-prefix skip lists, webroot-copy scoping) and confirm service-internal directories — `ops/`, `infra/`, `internal/`, `node_modules/`, `.git/`, CI config dirs — are excluded from whatever the script publishes to a public or shared target (webroot, package tarball, container image). A content-type filter alone does not stop an entire directory (e.g. `ops/` containing nginx server blocks or dev tooling) from being copied wholesale if the copy step is directory-recursive and no path-level exclusion exists.
    -   If the plan adds or moves a service-internal directory into the tree the deploy script copies, either extend the script's directory-exclusion list in the same plan, or record the gap as a known-limitation with a follow-up backlog item — same disposition as the content-type gap above.
    -   Rationale: a content-type filter (`*.py`, `*.md`) only stops individual files; it does not stop an entire directory from being pulled in by a recursive copy/sync (`rsync -a`, `cp -r`, tar of the repo root). Site-creation tasks in particular risk publishing `ops/`-style internal directories straight into the public webroot if the audit checks extensions but never checks directory scope.
...
11.6.  **Network Exposure Surfaces (tiered gate)**:
    ...
    -   Run `"${DATARIM_RUNTIME:?}/dev-tools/network-exposure-gate.sh" --task-description datarim/tasks/{TASK-ID}-task-description.md [--network-diff] --quiet`
        and apply the verdict per `${DATARIM_RUNTIME:?}/skills/network-exposure-baseline/SKILL.md` § Tiered Gate Rules:
...
12.  **Class B Public Surface Scan** (MANDATORY when Class A/B gate per `${DATARIM_RUNTIME:?}/skills/evolution/SKILL.md` classifies the task as **Class B** — operating-model / contract change):
    -   Enumerate ALL user-facing surfaces that reflect the new operating model. Minimum:
        ...
        -   `code/datarim/AGENTS.md`
        -   `Projects/Datarim/AGENTS.md` and `Projects/Datarim/README.md`
    ...
    -   **n-way runtime↔site sync (when task adds a NEW command/skill/agent)** — for every NEW artifact, public surface coverage MUST include:
        ...
        -   `Projects/Datarim/code/datarim/AGENTS.md` (commands/skills/agents table row, update count footer)
        -   `Projects/Datarim/code/datarim/README.md` (commands list mention, update count in the badge / description)
        These are the 4 surfaces required by the Public-surface ↔ runtime sync mandate (consumer AGENTS.md § Public-surface ↔ runtime sync). Asymmetric drift ("site is ahead of the framework" or vice versa) = a discoverability gap. Detector: `dev-tools/doc-fanout-lint.sh` + `tests/test-command-doc-coverage.bats`.
    ...
    -   **VERSION-bumping plans** (any class): copy `code/datarim/templates/version-consistency-fanout-checklist.md` into plan §5 (File matrix) and tick the applicable rows — it enumerates the canonical fanout targets (framework VERSION/AGENTS.md/README.md, workspace AGENTS.md/README.md, site config.php/changelog.php, docs ledger) so a VERSION bump does not silently miss one.
...
[ ] Installer/deploy-script content-type AND directory-level exclusion audit done (if plan touches install.sh / sync / deploy)?
...
[ ] If the plan touches a networking surface (compose ports / redis / postgres / systemd socket / firewall), `"${DATARIM_RUNTIME:?}/dev-tools/network-exposure-gate.sh"` was invoked and its verdict was applied: `hard_block` ⇒ § Network Exposure section present with Tier classification + verifier-pass AC; `advisory_warn` ⇒ § Risks one-liner; `skip` ⇒ nothing.
[ ] If Implementation Steps name a shipped framework runtime body (`skills/`/`agents/`/`commands/`/`templates/`) as an edit or create target, the § 6.5 History-agnostic runtime-body probe was run (task-ID-provenance gate per `${DATARIM_RUNTIME:?}/skills/evolution/history-agnostic-gate.md` + path exists/deprecation ladder per `${DATARIM_RUNTIME:?}/skills/plan-path-validator/SKILL.md`) and the verdict is annotated inline? (Shifts the history-agnostic gate left from `/dr-qa`/`/dr-compliance` to plan review, before approve.)
```
...
- `${DATARIM_RUNTIME:?}/templates/integration-checklist.md` — third-party-integration checklist for any task that adds, replaces, or modifies an integration with an external HTTP API, SDK, webhook target, OAuth provider, payment gateway, message queue, storage API, or LLM/STT/TTS endpoint. Reference from Step 6 (Technology Validation) when the task contains the `external API` keyword or introduces a new third-party dependency.
- `${DATARIM_RUNTIME:?}/templates/security-deps-upgrade-plan.md` — see `skills/security/SKILL.md`. Reference during Step 6 for dependency-CVE / framework-bump tasks.
- `${DATARIM_RUNTIME:?}/templates/infra-cost-reduction-checklist.md` — see `skills/infra-automation/SKILL.md`. Reference during Step 6 for VM/storage right-sizing or unused-resource cleanup.
<!-- gate:example-only -->
- For stack-specific scaffolds (e.g. NestJS, Django, Rails): see the relevant project's `AGENTS.md` or its per-project `${DATARIM_RUNTIME:?}/templates/` directory. The Datarim framework `${DATARIM_RUNTIME:?}/templates/` dir remains stack-agnostic — see `skills/evolution/stack-agnostic-gate.md`.
<!-- /gate:example-only -->
...
After plan generation, the planner agent MUST emit a CTA block ([definition](../skills/cta-format/SKILL.md)) per `${DATARIM_RUNTIME:?}/skills/cta-format/SKILL.md`.
