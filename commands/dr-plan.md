...
0.8. **SKILL**: Also load `$HOME/.claude/skills/immutability/SKILL.md` — read the `/dr-plan Rules` fragment for plan artefact immutability, V-AC parity (V-AC = verifiable acceptance criterion — an acceptance criterion paired with a concrete runnable check, [definition](../skills/v-ac-feasibility/SKILL.md)), and Return-to-Source routing.
...
12.  **Class B Public Surface Scan** (MANDATORY when Class A/B gate per `$HOME/.claude/skills/evolution/SKILL.md` classifies the task as **Class B** — operating-model / contract change):
    ...
    -   **VERSION-bumping plans** (any class): copy `code/datarim/templates/version-consistency-fanout-checklist.md` into plan §5 (File matrix) and tick the applicable rows — it enumerates the canonical fanout targets (framework VERSION/CLAUDE.md/README.md, workspace CLAUDE.md/README.md, site config.php/changelog.php, docs ledger) so a VERSION bump does not silently miss one.
