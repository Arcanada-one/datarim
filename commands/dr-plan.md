...
**Guarded deterministic auto-fix (L2+, automatic hook only).** After the Layer 1 floor emits findings carrying auto-fix metadata and before Layer 2/3 reviewer dispatch, the parent MAY apply a bounded fix through `dev-tools/self-verify-auto-fix.sh` per `skills/self-verification/SKILL.md` § Automatic Post-Step Guarded Auto-Fix. Eligibility is closed to `formatting`, allowlisted trailing-whitespace `lint`, and exact-map `obvious_typo` classes whose immutable class history proves a false-positive rate strictly below 30%; security, auth, secrets, and production-data findings are never auto-fixed. The parent applies one finding, then rerun the stage validators and deterministic floor before selecting another; the loop is capped at 32 runner invocations and every attempt is recorded. Missing or ineligible history stays advisory, and any failed transaction, lock conflict, or rollback-integrity failure makes the hook incomplete and non-advancing. Manual `/dr-verify` never enters this loop.

**Findings-only otherwise.** Every non-eligible or failed finding stays advisory; operator triages. Audit trail follows the manual path — write `datarim/qa/verify-{TASK-ID}-<stage>-<iter>.md` (append-only, `chmod a-w`) per the skill's Audit Log Writer only when Layer 2/3 ran (L2+); a pure-floor L2-tier run may skip the file and fold the floor verdict into the CTA summary line. The audit retains every original floor finding unchanged and adds one structured result for every runner invocation; `auto_fix_applied` is a derived count.
