...
4.  **Detailed Design (Phase 4)**:
    ...
    -   **Seam-vs-integration scope-boundary probe (MANDATORY for L3+ when the backlog one-liner bundles two discrete crate concerns).** Load `$HOME/.claude/skills/seam-vs-integration-boundary/SKILL.md`. If the one-liner mixes a seam/contract concern (define an interface / state machine / dispatcher in module X) with an integration/call-site concern (wire it into a command-line flag / call site / end-to-end path in module Y), flag the operator BEFORE component breakdown with the two-way choice — split into a foundation task + an integration follow-up, or scope the ACs so the integration is explicitly optional/deferred — and record the decision in the plan's Overview / Decisions section. Mechanical aid: `dev-tools/check-seam-integration-boundary.sh --task {TASK-ID} --report` surfaces candidate mixes (advisory only, never a hard block; the planner makes the final call). Cost: one grep + skim. Saving: avoids the integration half surfacing as out-of-scope at `/dr-qa` and being deferred as a late carry-forward note.
...
[ ] Adversarial pass run on the plan — "what would break this plan?" answered, not just "is the plan testable?" Load `$HOME/.claude/skills/adversarial-review/SKILL.md` (attack the premise, the strongest claim, the seams, reversibility) and, for any parser/lock/retry/state-machine/allocator step, `$HOME/.claude/skills/edge-case-hunter/SKILL.md` (boundary + failure-input sweep feeds the Validation Checklist). "No findings" is an alarm, not an all-clear — record the falsification attempt, not a bare pass. For a long or part-assembled plan, run `$HOME/.claude/skills/structure-review/SKILL.md` first (missing/contradictory sections, drifted counts, un-runnable step order).
