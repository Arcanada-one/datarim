...
0.7. **TASK-TYPE GUARD (research-task detection)**: Before running the product-PRD phases below, check whether this task is a **research / comparative-evaluation task** rather than a buildable product change. A PRD is a *product* requirements document; research tasks (technology selection, framework benchmarking, feasibility studies, literature surveys) produce an insights / decision artefact, not a shippable feature, and forcing them through the standard Problem→Scope→Technical-Approach→Success-Criteria structure yields a mis-shaped document.
    -   **Detection signals** (any one is sufficient):
        - the task ID uses the `RESEARCH` prefix (per `${DATARIM_RUNTIME:?}/skills/datarim-system/command-and-archive-rules.md` archive-subdir table);
    ...
    -   **On a positive detection**, emit a single advisory **WARNING** to the operator before continuing, and offer the **escalation hint**:
        ...
        - HINT: recommend the research path instead — run `/dr-plan` with the research-workflow skill (`${DATARIM_RUNTIME:?}/skills/research-workflow/SKILL.md`) so findings land in `datarim/insights/INSIGHTS-{task-id}.md`, and drive candidate elimination through `/dr-plan` § Research Kill-Criteria Checkpoint rather than a product Technical-Approach section. If the operator confirms a genuine product PRD is still wanted, proceed with the phases below.
    -   The guard is **advisory-only**: it never blocks. In auto-mode (see § /dr-auto Mode) resolve the confirm/decline through the Question Suppression Ladder; log the decision per `${DATARIM_RUNTIME:?}/skills/init-task-persistence/SKILL.md` § Q&A round-trip. On no detection, continue silently to Step 1.

0.8. **SKILL**: Also load `${DATARIM_RUNTIME:?}/skills/immutability/SKILL.md` — read the `/dr-prd Rules` fragment for PRD artefact immutability, V-AC parity, and Return-to-Source routing.
...
1.5. **Research External Context (Phase 1.3)** (L2+ only):
    ...
    -   Load `${DATARIM_RUNTIME:?}/skills/research-workflow/SKILL.md`.
    -   Spawn researcher agent (`${DATARIM_RUNTIME:?}/agents/researcher.md`) with task context: task ID, description, identified stack/dependencies from Phase 1.
    -   Agent creates `datarim/insights/INSIGHTS-{task-id}.md` from template `${DATARIM_RUNTIME:?}/templates/insights-template.md`.
...
2.  **Discovery Interview (Phase 1.5)**:
    ...
    -   Load `${DATARIM_RUNTIME:?}/skills/discovery/SKILL.md`.
    ...
    -   For L3-4 tasks, optionally invoke consilium skill (`${DATARIM_RUNTIME:?}/skills/consilium/SKILL.md`) for multi-perspective analysis of requirements.

2.5. **Stack Assumption Check** (new project/service or cross-domain work):
    -   Load `${DATARIM_RUNTIME:?}/skills/tech-stack/SKILL.md` and run the Trigger Classifier.
...
3.  **Explore Solutions (Phase 2)**:
    ...
    -   **Reuse-first check (MANDATORY when any candidate approach introduces a new non-secret, cross-project-reusable module — backend guard/interceptor/schema, frontend component/hook/util, shared config, client SDK wrapper, etc.)**: before finalizing candidate approaches, consult your organisation's shared-package catalog (for example a `catalog.json` in your shared-components repository, scoped as `@your-org/*`); if no catalog exists yet, browse the shared repository's packages directly and flag the gap. An existing package that already covers the need wins over a new local implementation. If no package fits, say so explicitly in the PRD (functional gap, not an unchecked assumption). If your workspace keeps a reuse-first mandate (for example `documentation/mandates/reuse-first-mandate.md` in the consumer workspace), it governs the details.
...
5.  **Generate PRD**:
    -   Use the structure from `${DATARIM_RUNTIME:?}/templates/prd-template.md`.
    -   Include: Problem Statement, Scope, Context Analysis, Technical Approach (Selected + Alternatives), Success Criteria, Risks.
    -   For infra/fleet PRDs, apply the `${DATARIM_RUNTIME:?}/templates/prd-template.md` § Deploy-Phase Verification Items labelling. `deploy-deferred` schedules a live check for its applicable phase; it is not a waiver or a presumed operator override. Retain the pending runtime check and its evidence gap. Apply the existing expectations deferral rules and live-evidence gate: unavailable evidence cannot become PASS, closure, or production authority.
...
5.5b. **Append-merge expectations checklist (L3-L4, mandatory)** per `${DATARIM_RUNTIME:?}/skills/expectations-checklist/SKILL.md`:
...
5.5. **Network Exposure Baseline (tiered gate)**:
    -   Read `${DATARIM_RUNTIME:?}/skills/network-exposure-baseline/SKILL.md` § Tier Model + § Tiered Gate Rules.
    -   Decide gate disposition by invoking the canonical executor:
        ...
        decision=$("${DATARIM_RUNTIME:?}/dev-tools/network-exposure-gate.sh" \
            ...
            --quiet)
        ```
        Resolve the path to `dev-tools/` via the runtime root (`${DATARIM_RUNTIME:?}/` symlinks to `code/datarim/`).
    -   Apply the decision:
...
6.  **Backlog Generation** (optional):
    ...
    -   **Determine prefix for generated items** per Unified Task Numbering (`${DATARIM_RUNTIME:?}/skills/datarim-system/SKILL.md`):
...
After PRD save, the architect agent MUST emit a CTA block ([definition](../skills/cta-format/SKILL.md)) per `${DATARIM_RUNTIME:?}/skills/cta-format/SKILL.md`.
