...
5.  **Generate PRD**:
    ...
    -   For infra/fleet PRDs, apply the `${DATARIM_RUNTIME:?}/templates/prd-template.md` § Deploy-Phase Verification Items labelling — mark any AC whose e2e verification requires a live broker, deployed host, or external webhook as `deploy-deferred` so `/dr-qa` expects partial + operator-override instead of flagging a coverage gap.
