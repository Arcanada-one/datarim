...
ctx_lock_acquire() {
  local lock="$1" tries=0 owner pid birth current stale
  owner="$lock/owner"
  while ! mkdir "$lock" 2>/dev/null; do
    if ctx_secure_regular "$owner" 600; then
      ...
      if [[ "$pid" =~ ^[0-9]+$ && -n "$birth" ]] && { [ "$current" != "$birth" ] || ! ctx_process_live "$pid"; }; then
        ...
        if mv "$lock" "$stale" 2>/dev/null; then rm -f "$stale/owner"; rmdir "$stale" 2>/dev/null || return 1; fi
        continue
      fi
    elif [ -d "$lock" ]; then
      # Cover a crash between mkdir and owner publication without racing a live
      # creator: only reclaim an ownerless lease after a bounded grace period.
      if [ "$(find "$lock" -maxdepth 0 -mmin +0 2>/dev/null | wc -l)" -gt 0 ]; then
        stale="${lock}.stale.$$.$RANDOM"
        if mv "$lock" "$stale" 2>/dev/null; then rmdir "$stale" 2>/dev/null || return 1; fi
        continue
      fi
    fi
