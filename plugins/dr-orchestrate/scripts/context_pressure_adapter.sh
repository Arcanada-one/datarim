...
normalize_codex_pressure() {
  local total window scaled quotient remainder max_exact
  ...
  max_exact=$((9 * 10**15))
  [ "$total" -le "$max_exact" ] && [ "$window" -le "$max_exact" ] || die 'token value too large'
...
resolve_codex_rollout_pressure() {
  local conversation="$1" root rollout candidate before after fd_path first values total window
  local -a matches=()
  ...
  while IFS= read -r -d '' candidate; do
    if grep -Fq -- "$conversation" "$candidate" 2>/dev/null; then matches[${#matches[@]}]="$candidate"; fi
  done < <(find "$root" -type f -name '*.jsonl' -print0 2>/dev/null)
  if [ "${#matches[@]}" -ne 1 ]; then die 'Codex rollout match is missing or ambiguous'; return 2; fi
  rollout="${matches[0]}"
