...
@test "bounded audit records lifecycle identifiers but not raw conversations" {
  ...
  [ "$(wc -l <"$audit")" -eq 4 ] && jq -e 'has("transaction") and has("snapshot_digest") and (has("conversation")|not)' "$audit" >/dev/null && ! rg -q 'conv-1|capability|RAW_' "$audit"
}
...
@test "HMAC capability bytes are never passed through OpenSSL argv" {
  run rg -n 'hexkey:|macopt.*cat' "$REPO_ROOT/plugins/dr-orchestrate/scripts/lib/context-window-state.sh" "$REPO_ROOT/plugins/dr-orchestrate/scripts/context_pressure_adapter.sh" "$REPO_ROOT/plugins/dr-orchestrate/scripts/context_window_controller.sh" "$REPO_ROOT/plugins/dr-orchestrate/scripts/context_window_setup.sh"
  [ "$status" -eq 1 ]
}
