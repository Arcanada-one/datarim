...
@test "codex large checked ratio does not lose integer precision" {
  total=58499999; total="${total}99999999"
  window=89999999; window="${window}99999999"
  run bash "$ADAPTER" normalize-codex-pressure --total "$total" --window "$window"
...
@test "Codex notify derives pressure only from its correlated private rollout" {
  ...
  [ "$(cat "$BATS_TEST_TMPDIR/actions")" = /compact ] && ! grep -ERq 'RAW_NOTIFY_CANARY|LAST_ASSISTANT_CANARY|UNRELATED_ROLLOUT_CANARY' "$DR_ORCH_CONTEXT_STATE" "$BATS_TEST_TMPDIR/actions"
}
