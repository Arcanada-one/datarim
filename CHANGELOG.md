...
- **Guarded deterministic auto-fix for the automatic post-step self-verification hook (TUNE-0140).** The automatic `post_step` hook wired into `/dr-prd`, `/dr-plan`, and `/dr-do` (L2+) may now apply a bounded, deterministic fix before dispatching model reviewers. The read-only floor (`dev-tools/dr-verify-floor.sh`) emits closed auto-fix metadata for missing-final-newline `formatting`, trailing-whitespace `lint`, and exact-map `obvious_typo` findings on the exact stage artifact; a new local runner (`dev-tools/self-verify-auto-fix.sh`) applies at most one finding per floor generation with strict integer `10 * false_positive < 3 * total` (<30%) gating drawn only from the fixed immutable namespace `datarim/qa/self-verification-auto-fix-history/records/`, task-stage locking, durable prepared/terminal journals, atomic replacement, rollback, and replay. Security, authentication, secrets, production-data, model-authored, and unknown-class findings are never auto-fixed; the manual `/dr-verify` command stays findings-only and cannot reach the runner; reviewer prose is never executable authority. Any transaction uncertainty is non-advancing. Ships inert (no positive history records). Focused coverage: `tests/self-verification-auto-fix-policy.bats` (38) + `tests/self-verification-auto-fix-wiring.bats` (12).
