...
- **`/dr-archive` Step 0.4** — mandatory "Expectations re-validation + anti-deferral gate", inserted between the network gate (0.3) and reflection (0.5). Placed before reflection on purpose: reflection's follow-up heuristic must not launder a self-inflicted loose end into a backlog item ahead of the gate.
...
- **Regression suites** `tests/check-deferral-prose.bats` (9), `tests/check-expectations-override-authorship.bats` (6), `tests/dr-archive-step04-anti-deferral.bats` (3), `tests/dr-compliance-deferral-gate.bats` (3).
