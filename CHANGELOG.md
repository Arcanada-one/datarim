...
- **Cross-runtime `/dr-auto` activation smoke (`dev-tools/check-dr-auto-cross-runtime.sh`).** A pure-bash preflight that proves the `/dr-auto` autonomous-mode activation contract is runtime-agnostic (Claude Code / Codex CLI / Cursor): it asserts (1) `auto-mode-marker.sh reassert` writes a marker whose path `resolve` reports back, (2) `subagent-active` decides `active`/`non-auto` with `DATARIM_AUTO_MODE` unset (a spawned Codex/Cursor subagent does not inherit the shell env var), and (3) `dev-tools/rules/fb-rules.yaml` declares a non-empty `hard_gated_actions` list. Modes `--check`/`--report`/`--help`; exits `0` pass / `1` fail / `2` precondition-or-usage. Locates the marker via the tool's own `resolve` verb rather than a hard-coded path, so it tracks the per-task marker layout. Documented in `documentation/how-to/multi-runtime.md` § Cross-runtime `/dr-auto` smoke; 6 focused Bats cases in `tests/check-dr-auto-cross-runtime.bats`. (TUNE-0292)
