...
- **`install.sh --enable-codex-prompts` — Codex `/prompts:dr-*` slash-command parity.** Opt-in, default-off flag that mirrors each shipped `commands/dr-*.md` into `$CODEX_DIR/prompts/dr-<name>.md` via the new `mirror_prompts_target()`, so Codex CLI exposes the Datarim commands as `/prompts:dr-status`, `/prompts:dr-do`, … (empirically verified on codex-cli 0.133.0). Flat copies (not symlinks) for Windows/FAT parity with `--with-cursor`; the copies stay byte-identical to source so the verified `/prompts:` behaviour is preserved. The mirror is a deterministic, ownership-tracked reconcile — content-gated copy (no mtime churn on re-run), namespace-conflict detection (a pre-existing `dr-*.md` this mirror did not author is skipped + warned, never overwritten; non-`dr-*` files are never touched), and prune of managed files whose source command was removed (a stale executable slash-command would otherwise keep running old behaviour). Ownership is tracked in the dotfile `$CODEX_DIR/prompts/.datarim-managed`. Independent of `--no-codex-ux`. Ships behind an accepted-risk (deferred-validation) posture: Codex documents the `/prompts:` mechanism as legacy and may retire it, hence opt-in. Bats T51-T54 (mirror + non-dr exclusion + manifest + idempotency + opt-out + namespace conflict + dry-run + `--help` advertisement). Documented in `documentation/how-to/multi-runtime.md`. (TUNE-0306)
