...
- **Brand-hygiene policy for absorbed external skills.** The `evolution` skill now carries a mandatory
  ...
  TUNE-0151). (TUNE-0153)
- **Reusable collision-safe task-prefix rename (`dev-tools/rename-task-prefix.sh`).** Generalises the one-off ADR->ADSR rename into a repeatable safe operation: a bash CLI over a pure, offline Python classifier (`dev-tools/lib/prefix-rename-classify.py`) that decides per line whether each `OLD-NNNN` token is a task reference (rename) or a homograph to protect (keep) -- the case a blind `sed` once corrupted across 33 rank-1 mandate references. Anchors are literal substrings, never regex (avoids the earlier UTF-8 middle-dot and slash-bearing failures); a line matching any `--exclude-anchor` is protected wholesale, and a `--collision-number` renames only on a line carrying an `--include-anchor`. Supports content rewrite, `--rename-files` (git-mv + internal-ref rewrite, path-exclude-aware, orphan-free), an optional `--registry-file` row assertion, a non-destructive `--dry-run` default, and a `--verify` V-AC sweep that fails on any unrenamed residue. shellcheck-clean; 7-case focused Bats matrix (V-AC-01..07). (TUNE-0368)
