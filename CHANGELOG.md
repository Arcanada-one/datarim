...
- **`skills/seam-vs-integration-boundary/SKILL.md`** _(NEW)_ — plan-time scope-boundary pattern guidance. When an L3+ backlog one-liner bundles a seam/contract concern (define an interface/dispatcher in module X) with an integration/call-site concern (wire it into a caller in module Y), `/dr-plan` flags the operator to either split into a foundation task + an integration follow-up, or scope the ACs so the integration is explicitly optional/deferred — closing the failure mode where the integration half surfaces as out-of-scope only at `/dr-qa` and gets deferred as a late carry-forward note. Ships with an advisory detector `dev-tools/check-seam-integration-boundary.sh` (narrow deterministic heuristic; flags seam-signal × integration-signal co-occurrence in a one-liner; advisory by default, `--strict` exits 3; `--task <ID>` reads the one-liner from the backlog) and a new `/dr-plan` Step 4 seam-vs-integration scope-boundary probe (mandatory for L3+). Skills count 67 → 68. New regression test `tests/check-seam-integration-boundary.bats` (14 cases). Class A evolution proposal from `reflection-ARAS-0003` Proposal 3 (approved 2026-05-16). (TUNE-0239)
