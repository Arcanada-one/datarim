...
- **`semgrep` and `shellcheck-extracted` un-redded on `main`.** Two more required checks were failing on `main` alongside `task-id-gate`, all three introduced by the same batch of parallel relands. (1) `semgrep` — three newly-landed scripts (`capture-framework-version-baseline.sh`, `check-framework-version-accountability.sh`, `check-strategist-gate-record.sh`) open with a bare top-level strict-mode `IFS=$'\n\t'`, which `bash.lang.security.ifs-tampering` flags. Handled with the repo's existing sanctioned annotation, verbatim from `scripts/dr-plugin.sh:26` and `scripts/datarim-doctor.sh:26` — a rule-scoped `# nosemgrep` naming the reason (process-wide strict-mode IFS in a single-script process), not a blanket suppression. Sibling scripts using only inline `IFS= read` are not flagged and were left alone. (2) `shellcheck-extracted` — three fenced ```bash blocks documenting those helpers' invocation (`commands/dr-init.md`, `commands/dr-do.md`, `commands/dr-qa.md`) carry `<workspace-root>` / `<framework-repo>` placeholders that shellcheck parses as redirections (SC1073/SC1072, hard errors). Each block now opens with the extractor's own `# noshellcheck-extract` marker, the mechanism `commands/dr-continue.md` already uses for placeholder-bearing blocks. Verified by re-running the extractor over every `commands/*.md`: 5 blocks extracted, shellcheck exit 0. (TUNE-0543)

- **`task-id-gate` un-redded on `main`.** The brand-hygiene policy addition to `skills/evolution/SKILL.md` shipped a provenance block naming four task IDs inline, which CLAUDE.md Critical Rule 8 forbids on the shipped instruction surface (`skills`/`agents`/`commands`/`templates`) — so the `task-id-gate` CI job has been failing on `main` (`T11: skills/ scope is gate-clean` red), blocking every downstream PR. Rewritten in the same abstract, task-ID-free style the file's three other `**Source:**` lines already use, citing `documentation/how-to/evolution-log.md` for provenance. No `<!-- gate:history-allowed -->` marker was needed: the escape hatch exists for illustrative example blocks, not for provenance prose that paraphrases cleanly. Gate now clean on all four scopes; `tests/task-id-gate.bats` 17/17. (TUNE-0543)
